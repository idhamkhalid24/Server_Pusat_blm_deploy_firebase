/* === Inline script 1 dari file asli === */
import { initializeApp } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-app.js";
  import {
    getFirestore, enableIndexedDbPersistence, doc, getDoc, getDocFromServer, setDoc, addDoc, deleteDoc, collection,
    getDocs, getDocsFromServer, onSnapshot, serverTimestamp, query, where, orderBy, limit, Timestamp
  ...
  // Omzet kasir diambil dari Firebase admin, sedangkan Ops Toko + QRIS + Tabungan
  ...
  const SESSION_KEY = 'rocky_firebase_session_v1';
  const PAGE_KEY = 'rocky_firebase_last_page_v1';
  ...
  // Isi VAPID key kalau ADMIN.html dipakai di browser/PWA. Untuk APK Android, token FCM dikirim native lewat window.saveNativeAdminFcmToken().
  ...
  let cachedTransactions = [], cachedAttendance = [], cachedUsers = [], cachedAuditLogs = [], cachedClosings = [], cachedManualBonuses = [];
  let adminNotifBootstrapped = false;
  let adminNotifSeenIds = new Set();
  let cachedBonusSettings = { transactionBonusRate: 0.015, closingBonusPerMinute: 100 };
  ...
  let unsubTransactions = null, unsubAttendance = null, unsubUsers = null, unsubAuditLogs = null, unsubClosings = null, unsubBonusSettings = null, unsubManualBonuses = null, unsubRismaManualClosing = null, unsubHeaderGuideNote = null, unsubStaffDailyHomeNote = null, unsubReceiptTextSettings = null, rangeStartDate = null, rangeEndDate = null;
  ...
  const TX_SYNC_LIMIT_ADMIN = 800, TX_SYNC_LIMIT_STAFF = 400, ATT_SYNC_LIMIT_ADMIN = 800, ATT_SYNC_LIMIT_STAFF = 400, AUDIT_SYNC_LIMIT = 40, CLOSING_SYNC_LIMIT = 120, MANUAL_BONUS_SYNC_LIMIT = 250, HISTORY_PAGE_STEP = 30, INACTIVE_USER_STEP = 30;
  const ATTENDANCE_PAGE_STEP = 30;
  const CLOSING_DEADLINE_HOUR = 18, CLOSING_DEADLINE_MINUTE = 0, DEFAULT_CLOSING_BONUS_PER_MINUTE = 100, DEFAULT_TRANSACTION_BONUS_RATE = 0.015;
  const RISMA_MANUAL_CLOSING_DOC_ID = '__risma_manual_closing';
  const RISMA_MANUAL_CLOSING_COLLECTION = 'closings';
  let staffSwipeStartX = 0, staffSwipeStartY = 0, staffSwipeStartT = 0;
  ...
  function askPin(message = 'Masukkan PIN:', title = 'Konfirmasi PIN') {
    return new Promise((resolve) => {
      ...
      wrap.innerHTML = `
        <div class="modal-sheet" style="max-width:360px;border-radius:22px;padding:18px">
  ...
  function loadAdminNotifSeenIds() {
    try {
      ...
      adminNotifSeenIds = new Set(Array.isArray(raw) ? raw.slice(-250) : []);
    } catch (_) {
      adminNotifSeenIds = new Set();
    }
  }
  function saveAdminNotifSeenIds() {
    try { localStorage.setItem(ADMIN_NOTIF_SEEN_KEY, JSON.stringify([...adminNotifSeenIds].slice(-250))); } catch (_) {}
  }
  ...
  function getTransactionPaymentBadge(tx = {}) {
    ...
    const cls = method === 'qris_transfer' ? 'badge-blue' : '';
    return `<span class="badge ${cls}" style="font-size:8.5px;letter-spacing:.02em">${escapeHtml(label)}</span>`;
  }
  ...
  async function saveNativeAdminFcmToken(token, platform = 'android') {
    ...
    try {
      ...
      showToast('✓ Token notifikasi admin tersimpan');
      return true;
    } catch (e) {
  ...
  const getAttendanceCacheKey = (username) => `rocky_attendance_cache_${sanitizeUsername(username)}`;
  function saveAttendanceCache(username, payload) {
    if (!username) return;
    localStorage.setItem(getAttendanceCacheKey(username), JSON.stringify({
      dateKey: payload?.dateKey || toDateKey(nowDate()),
      createdAtMs: Number(payload?.createdAtMs || Date.now()),
      cachedAt: Date.now()
    }));
  }
  function readAttendanceCache(username) {
    if (!username) return null;
    try {
      const raw = localStorage.getItem(getAttendanceCacheKey(username));
      if (!raw) return null;
      const parsed = JSON.parse(raw);
      if (!parsed || parsed.dateKey !== toDateKey(nowDate())) return null;
      return parsed;
    } catch (e) { return null; }
  }
  function clearAttendanceCache(username) {
    if (!username) return;
    localStorage.removeItem(getAttendanceCacheKey(username));
  }
  function preloadAttendanceCache() {
    if (!currentUser || isAdmin()) return false;
    const cached = readAttendanceCache(currentUser.username);
    if (!cached) return false;
    const exists = cachedAttendance.some(a => a.__cached && a.dateKey === cached.dateKey && a.user === currentUser.username);
    if (!exists) {
      cachedAttendance = [{
        id: `cached_${currentUser.username}_${cached.dateKey}`,
        user: sanitizeUsername(currentUser.username),
        name: currentUser.name,
        dateKey: cached.dateKey,
        createdAtMs: cached.createdAtMs || Date.now(),
        __cached: true
      }, ...cachedAttendance.filter(a => !a.__cached)];
    }
  ...
  function upsertAttendanceRecord(rec) {
    ...
    cachedAttendance = sortByCreatedDesc([
      { ...rec, user, dateKey },
      ...cachedAttendance.filter(a => !(sanitizeUsername(a.user) === user && extractDateKey(a) === dateKey))
    ]);
  }
  ...
  async function findExistingAttendanceToday(username, dateKey = toDateKey(nowDate())) {
    const user = sanitizeUsername(username);
    if (!user || !dateKey) return null;

    try {
      const snap = await getDocFromServer(doc(db, 'attendance', getAttendanceDocId(user, dateKey)));
      if (snap.exists()) {
        const rec = { id: snap.id, ...snap.data() };
        if (!isRecordDeleted(rec)) return rec;
      }
    } catch (e) {
      console.warn('Cek absen by doc id gagal:', e?.message || e);
    }

    try {
      const q = query(collection(db, 'attendance'), where('user', '==', user), where('dateKey', '==', dateKey), limit(1));
      const snap = await getDocs(q);
      const rec = snap.docs.map(d => ({ id: d.id, ...d.data() })).find(a => !isRecordDeleted(a));
      return rec || null;
    } catch (e) {
      console.warn('Cek absen legacy gagal:', e?.message || e);
      return null;
    }
  }

  async function syncTodayAttendanceFromServer() {
    if (!currentUser || isAdmin()) return null;
    const user = sanitizeUsername(currentUser.username);
    const todayKey = toDateKey(nowDate());
    const rec = await findExistingAttendanceToday(user, todayKey);
    if (rec) {
      const createdAtMs = getRecordTimeMs(rec) || Number(rec.createdAtMs || Date.now());
      saveAttendanceCache(user, { dateKey: todayKey, createdAtMs });
      upsertAttendanceRecord({ ...rec, user, name: rec.name || currentUser.name, dateKey: todayKey, createdAtMs });
  ...
  const getRoleLabel = () => currentUser ? getRoleDisplayName(currentUser.role || 'staff') : (isAdmin() ? 'Admin' : 'Staff');
  ...
  function getUserDataScopeLabel() {
    return isAdmin() ? getSelectedUserLabel() : 'Data Saya';
  }
  ...
  function getStaffBonusUsers() {
    return activeUsers().filter(isStaffBonusUser);
  }
  ...
  function getVisibleTransactions() {
    const visible = cachedTransactions.filter(t => !isRecordDeleted(t));
    if (!isAdmin() && currentUser?.username) {
      const me = sanitizeUsername(currentUser.username);
      return visible.filter(t => sanitizeUsername(t.user) === me);
    }
    return visible;
  }
  function getVisibleAttendance() {
    return cachedAttendance.filter(a => !isRecordDeleted(a));
  }
  ...
  function getAdminDashboardData() {
    ...
    return { activeToday: todayUsers.size, userCount: activeUsers().length };
  }
  function renderAdminClosingPanelMaybe(force = false) {
    ...
    const hash = [targetValue, closing?.id || '', closing?.closed === true ? '1' : '0', closing?.closingTime || '', closing?.manualClosingTime || '', closing?.editedAtMs || '', closing?.canceledAtMs || '', cachedUsers.length, getClosingBonusPerMinute(), userBonusHash, manualClosingHash].join('|');
  ...
  function renderAdminDashboardPanels() {
    ...
    const noteHash = `${cachedStaffDailyHomeNote?.enabled === false ? '0' : '1'}|${cachedStaffDailyHomeNote?.updatedAtMs || 0}|${String(cachedStaffDailyHomeNote?.note || '').slice(0, 120)}`;
    const hash = `${stats.activeToday}|${stats.userCount}|${noteHash}`;
    ...
    board.innerHTML = `
      ...
      ${renderStaffDailyHomeNoteHomeCard()}
...
`;
  ...
  const toggleTheme = () => {
    ...
    applyTheme(next);
    document.querySelectorAll('.theme-btn').forEach(b => b.innerHTML = next === 'dark' ? '<i class="fas fa-moon"></i>' : '<i class="fas fa-sun"></i>');
    document.querySelectorAll('input[type="date"].g-input').forEach(el => el.style.colorScheme = next);
  };
  applyTheme(getTheme());

  const VIEW_MODE_KEY = 'rocky_admin_view_mode_v1';
  function getAppViewMode() {
    try {
      const saved = localStorage.getItem(VIEW_MODE_KEY);
      if (saved === 'desktop' || saved === 'android') return saved;
    } catch (_) {}
    return window.matchMedia && window.matchMedia('(max-width: 759px)').matches ? 'android' : 'desktop';
  }
  function applyAppViewMode(mode, silent = false) {
    const next = mode === 'android' ? 'android' : 'desktop';
    try { localStorage.setItem(VIEW_MODE_KEY, next); } catch (_) {}
    document.documentElement.setAttribute('data-view-mode', next);
    document.querySelectorAll('.view-mode-btn').forEach(btn => {
      btn.innerHTML = next === 'android' ? '<i class="fas fa-mobile-screen-button"></i>' : '<i class="fas fa-desktop"></i>';
      btn.title = next === 'android' ? 'Mode Android aktif' : 'Mode Desktop aktif';
      btn.setAttribute('aria-label', next === 'android' ? 'Mode Android aktif' : 'Mode Desktop aktif');
    });
    if (window.syncKasirPintarLayout) window.syncKasirPintarLayout();
    if (!silent) showToast(next === 'android' ? 'Mode Android aktif' : 'Mode Desktop aktif');
  }
  function toggleAppViewMode() {
    applyAppViewMode(getAppViewMode() === 'android' ? 'desktop' : 'android');
  }
  applyAppViewMode(getAppViewMode(), true);
  ...
  async function fetchLatestBonusBeforeTransaction(username) {
    ...
    if (bonusSnap?.exists()) {
      ...
      cachedBonusSettings = {
        transactionBonusRate: Number.isFinite(Number(data.transactionBonusRate)) ? Number(data.transactionBonusRate) : DEFAULT_TRANSACTION_BONUS_RATE,
        closingBonusPerMinute: Number.isFinite(Number(data.closingBonusPerMinute)) ? Number(data.closingBonusPerMinute) : DEFAULT_CLOSING_BONUS_PER_MINUTE
      };
    }
    ...
    if (sanitizeUsername(currentUser?.username) === safeUsername) {
      currentUser = {
        ...
        name: freshUser.name,
        pin: String(freshUser.pin || currentUser?.pin || ''),
  ...
  async function saveBonusSettings() {
    ...
    if (!pin) return;
    if (String(pin) !== String(currentUser.pin)) return showToast('PIN salah!', true);
    setLoading(true);
    try {
      const payload = {
      ...
      console.error(e);
      showToast('Gagal simpan kontrol bonus: ' + (e.code || e.message || 'cek Firestore'), true);
    }
    setLoading(false);
  }
  ...
  function renderManualBonusCard() {
    ...
    const allRows = getMonthlyManualBonuses(null, month);
    const rows = allRows.slice(0, 30);
    const totalManual = getMonthlyManualBonus(null, month);
...
Masukkan PIN admin:`);
      ...
      upsertManualBonusRecord({ id: ref.id, ...payload });
      ...
      console.error(e);
      showToast('Gagal simpan bonus manual: ' + (e.code || e.message || 'cek Firestore'), true);
    } finally {
  ...
  async function deleteManualBonus(id) {
    ...
    const pin = await askPin(`Hapus catatan bonus manual ${item.name || item.user || ''}?\nNilai: ${amount < 0 ? '-' : '+'} Rp ${formatRupiah(Math.abs(amount))}\nBulan: ${item.monthKey || '-'}\n\nMasukkan PIN admin:`);
    if (!pin) return;
    if (String(pin) !== String(currentUser.pin)) return showToast('PIN salah!', true);
    ...
    try {
      ...
      console.error(e);
      showToast('Gagal hapus bonus manual: ' + (e.code || e.message || 'cek Firestore'), true);
    }
    setLoading(false);
  }
  ...
  function getClosingDelayMinutes(date = new Date()) {
    const wib = getWIBDate(date);
    const currentMinutes = (wib.hour * 60) + wib.minute;
    const deadlineMinutes = (CLOSING_DEADLINE_HOUR * 60) + CLOSING_DEADLINE_MINUTE;
    return Math.max(0, currentMinutes - deadlineMinutes);
  }
  function formatClosingDeadline() {
    return `${String(CLOSING_DEADLINE_HOUR).padStart(2,'0')}:${String(CLOSING_DEADLINE_MINUTE).padStart(2,'0')} WIB`;
  }
  ...
  function getDeadlineTimeLabel() {
    return `${String(CLOSING_DEADLINE_HOUR).padStart(2,'0')}:${String(CLOSING_DEADLINE_MINUTE).padStart(2,'0')}`;
  }
  function getClosingTimeFromDelay(delayMinutes) {
    const delay = Number(delayMinutes || 0);
    if (!Number.isFinite(delay) || delay <= 0) return '';
    const total = (CLOSING_DEADLINE_HOUR * 60) + CLOSING_DEADLINE_MINUTE + delay;
  ...
  function getClosingDisplayTime(closing = getTodayClosing()) {
    ...
    const deadlineTime = getDeadlineTimeLabel();
    if (savedClosingTime && savedClosingTime !== deadlineTime) return savedClosingTime;
    const delayTime = getClosingTimeFromDelay(closing.delayMinutes);
  ...
  function getClosingDelayMinutesFromTime(hour, minute) {
    const currentMinutes = (Number(hour) * 60) + Number(minute);
    const deadlineMinutes = (CLOSING_DEADLINE_HOUR * 60) + CLOSING_DEADLINE_MINUTE;
    return Math.max(0, currentMinutes - deadlineMinutes);
  }
  function getStaffClosingUsers(dateKey = toDateKey(nowDate())) {
    return activeUsers().filter(u => isClosingEligibleUser(u, dateKey));
  }
  ...
  function isClosingEligibleUser(userOrUsername, dateKey = toDateKey(nowDate())) {
    ...
    if (!uname || ADMIN_USERNAMES.includes(uname) || sanitizeUsername(user.role || 'staff') === 'admin') return false;
    // Aturan closing: karyawan harian boleh diclosing tanpa absen.
    // Staff biasa hanya boleh diclosing kalau sudah absen di tanggal tersebut.
  ...
  function buildClosingBonusByUser(delayMinutes, targetUsername = null, dateKey = toDateKey(nowDate())) {
    ...
    const allStaffTargets = getStaffClosingUsers();
    ...
    const delay = Number(delayMinutes || 0);
    const bonusUsers = eligibleUsers.filter(u => !isDailyEmployee(u) && hasUserAttendanceOnDate(u.username, dateKey));
  ...
  function isAtOrAfterClosingDeadline(date = new Date()) {
    const wib = getWIBDate(date);
    const currentMinutes = (wib.hour * 60) + wib.minute;
    const deadlineMinutes = (CLOSING_DEADLINE_HOUR * 60) + CLOSING_DEADLINE_MINUTE;
  ...
  function renderClosingStatusHtml(compact = false) {
    ...
    if (!closing) {
      ...
      return `<div style="display:flex;flex-direction:column;gap:8px;padding:${compact?'10px 11px':'12px 13px'};border:1px solid ${showEstimate?'rgba(255,196,107,.24)':'var(--border)'};border-radius:13px;background:${showEstimate?'var(--amber-dim)':'var(--surface)'}">
        ...
        ${showEstimate ? `<div style="font-size:10px;color:var(--text-soft);line-height:1.35;border-top:1px solid rgba(255,196,107,.16);padding-top:7px">Estimasi ini otomatis naik setiap menit setelah 18:00 WIB sampai admin melakukan closing.</div>` : ''}
      </div>`;
    }
...
${targetUsername ? 'Hanya user ini yang terkunci. User lain tetap bisa transaksi.' : 'Hanya user yang sudah absen atau karyawan harian yang masuk target closing.'}
Masukkan PIN admin:`);
    if (!pin) return;
    if (String(pin) !== String(currentUser.pin)) return showToast('PIN salah!', true);
    const currentClosingTime = getCurrentWIBTimeLabel(nowDate());
    const delayMinutes = getClosingDelayMinutes(nowDate());
    ...
    if (targetUsername && !targetUsers.length) return showToast('User belum absen / bukan karyawan harian, jadi tidak perlu closing.', true);
    if (!targetUsername && !targetUsers.length) return showToast('Belum ada user yang sudah absen atau karyawan harian untuk closing.', true);
...
${users.length ? '' : 'Catatan: staff belum absen tidak ikut diclosing; karyawan harian tetap bisa diclosing tanpa bonus.'}`)) return;
    setLoading(true);
    try {
      ...
      await setDoc(doc(db, 'closings', docId), {
        ...
        canceledAtMs: null,
        editedAtMs: null,
        deadline: formatClosingDeadline(),
        delayMinutes,
      ...
      await logAudit('closing_day', { dateKey: today, scope: targetUsername ? 'user' : 'global', user: targetUsername || null, closingTime: currentClosingTime, delayMinutes, bonusPerUser, totalUsers: users.length, totalBonus });
  ...
  async function handleEditClosingTime() {
    ...
    if (!pin) return;
    if (String(pin) !== String(currentUser.pin)) return showToast('PIN salah!', true);
    const delayMinutes = getClosingDelayMinutesFromTime(parsed.hour, parsed.minute);
    const { users, bonusByUser, bonusPerMinuteByUser, bonusPerUser, totalBonus } = buildClosingBonusByUser(delayMinutes, targetUsername, closing.dateKey || toDateKey(nowDate()));
...
${lockedByGlobal ? 'Hanya user ini yang dibuka dari closing global. Bonus closing user ini tidak dihitung.' : 'Bonus closing target ini tidak akan dihitung.'}`)) return;
  ...
  async function login() {
    ...
    try {
      ...
      currentUser = { username: sanitizeUsername(user.username || username), name: user.name, pin: String(user.pin), role: user.role || 'staff', transactionBonusRate: user.transactionBonusRate ?? (isDailyRole(user.role || 'staff') ? user.dailyBonusRate : null), transactionBonusPercent: user.transactionBonusPercent ?? (isDailyRole(user.role || 'staff') ? user.dailyBonusPercent : null), closingBonusPerMinute: user.closingBonusPerMinute ?? null, dailyBonusRate: Number(user.dailyBonusRate || 0), dailyBonusPercent: Number(user.dailyBonusPercent || 0), active: isUserActive(user), deleted: false };
      cachedTransactions = [];
      cachedAttendance = [];
      cachedAuditLogs = [];
      cachedClosings = [];
  ...
  function logout() {
    ...
    adminNotifBootstrapped = false;
    adminNotifSeenIds = new Set();
    if (logoutUsername) clearAttendanceCache(logoutUsername);
    cachedTransactions = [];
    cachedAttendance = [];
    cachedUsers = [];
    cachedAuditLogs = [];
    ...
    cachedHeaderGuideNote = { id: HEADER_GUIDE_NOTE_DOC_ID, note: DEFAULT_HEADER_GUIDE_NOTE, updatedAtMs: 0, updatedBy: '', updatedByName: '' };
      cachedStaffDailyHomeNote = { id: STAFF_DAILY_NOTE_DOC_ID, note: DEFAULT_STAFF_DAILY_NOTE, enabled: true, updatedAtMs: 0, updatedBy: '', updatedByName: '' };
    cachedReceiptTextSettings = { id: RECEIPT_TEXT_DOC_ID, ...DEFAULT_RECEIPT_TEXT_SETTINGS, updatedAtMs: 0, updatedBy: '', updatedByName: '' };
    cachedBonusSettings = { transactionBonusRate: DEFAULT_TRANSACTION_BONUS_RATE, closingBonusPerMinute: DEFAULT_CLOSING_BONUS_PER_MINUTE };
    attendanceReady = false;
    ...
    if (unsubStaffDailyHomeNote) { unsubStaffDailyHomeNote(); unsubStaffDailyHomeNote = null; }
  ...
  function restoreSession() {
    try {
      ...
      if (!isAdminAccount(currentUser, currentUser.username)) {
        localStorage.removeItem(SESSION_KEY);
        localStorage.removeItem(PAGE_KEY);
        currentUser = null;
        return false;
  ...
  function bindRealtimeData() {
    ...
    // Jangan pakai orderBy('createdAt') di query staff karena data lama/migrasi
    // bisa belum konsisten field createdAt-nya. Itu bikin transaksi staff kadang
    // kosong setelah clear cache/data, padahal admin masih bisa lihat.
    const tq = isAdmin()
      ? query(collection(db, 'transactions'), limit(TX_SYNC_LIMIT_ADMIN))
      : query(collection(db, 'transactions'), where('user', '==', sanitizeUsername(currentUser.username)), limit(TX_SYNC_LIMIT_STAFF));
    ...
    const aq = isAdmin()
      ? query(collection(db, 'attendance'), limit(ATT_SYNC_LIMIT_ADMIN))
      : query(collection(db, 'attendance'), where('user', '==', sanitizeUsername(currentUser.username)), limit(ATT_SYNC_LIMIT_STAFF));
    unsubAttendance = onSnapshot(aq, (snap) => {
      ...
      if (!isAdmin()) {
        const today = toDateKey(nowDate());
        const todayRec = cachedAttendance.find(a => !isRecordDeleted(a) && extractDateKey(a) === today && sanitizeUsername(a.user) === currentUser.username);
        if (todayRec) {
          const createdAtMs = getRecordTimeMs(todayRec) || Date.now();
          saveAttendanceCache(currentUser.username, { dateKey: today, createdAtMs });
        } else {
          clearAttendanceCache(currentUser.username);
        }
      }
      attendanceReady = true;
      refreshCurrentPage();
      if (!isAdmin() && !hasClockedInToday()) syncTodayAttendanceFromServer();
    }, (err) => { attendanceReady = true; console.error(err); refreshCurrentPage(); if (!isAdmin()) syncTodayAttendanceFromServer(); });

    if (!isAdmin()) syncTodayAttendanceFromServer();
    ...
    unsubStaffDailyHomeNote = onSnapshot(doc(db, 'closings', STAFF_DAILY_NOTE_DOC_ID), (snap) => {
      cachedStaffDailyHomeNote = snap.exists()
        ? { id: snap.id, note: DEFAULT_STAFF_DAILY_NOTE, enabled: true, ...(snap.data() || {}) }
        : { id: STAFF_DAILY_NOTE_DOC_ID, note: DEFAULT_STAFF_DAILY_NOTE, enabled: true, updatedAtMs: 0, updatedBy: '', updatedByName: '' };
      refreshCurrentPage();
    }, (err) => console.error('Catatan home staff/harian listener gagal:', err));
    ...
    const mbq = isAdmin()
      ? query(collection(db, 'manualBonuses'), limit(MANUAL_BONUS_SYNC_LIMIT))
      : query(collection(db, 'manualBonuses'), where('user', '==', sanitizeUsername(currentUser.username)), limit(MANUAL_BONUS_SYNC_LIMIT));
    ...
    if (isAdmin()) {
      ...
      cachedUsers = currentUser ? [{ username: currentUser.username, name: currentUser.name, role: currentUser.role || 'staff', transactionBonusRate: currentUser.transactionBonusRate ?? null, transactionBonusPercent: currentUser.transactionBonusPercent ?? null, closingBonusPerMinute: currentUser.closingBonusPerMinute ?? null, dailyBonusRate: Number(currentUser.dailyBonusRate || 0), dailyBonusPercent: Number(currentUser.dailyBonusPercent || 0), active: true }] : [];
      cachedAuditLogs = [];
    }
  ...
  async function refreshCurrentPage() {
    ...
    else if (currentPage === 'history') { if (isAdmin()) await renderHistory(); else renderStaffTodayHistoryPage(); }
    else if (currentPage === 'attendance') renderAttendanceAdminPage();
    else if (currentPage === 'report') await renderReport();
    else if (currentPage === 'settings') await renderSettingsSummary();
  ...
  function hasClockedInToday() {
    const today = toDateKey(nowDate());
    const me = sanitizeUsername(currentUser?.username || '');
    return cachedAttendance.some(a => !isRecordDeleted(a) && extractDateKey(a) === today && (!me || sanitizeUsername(a.user) === me));
  }
  function canTransactNow() {
    if (isAdmin()) return true;
    if (isTransactionsClosedToday()) return false;
    if (isDailyEmployee(currentUser)) return true;
    if (!attendanceReady) return false;
    return hasClockedInToday();
  }
  function getTodayAttendanceRecord() {
    const today = toDateKey(nowDate());
    return sortByCreatedDesc(cachedAttendance.filter(a => !isRecordDeleted(a) && extractDateKey(a) === today))[0] || null;
  }
  ...
  async function saveHeaderGuideNote() {
    ...
    if (!pin) return;
    if (String(pin) !== String(currentUser?.pin || '')) return showToast('PIN admin salah!', true);
    setLoading(true);
    try {
      const payload = {
  ...
  function renderReceiptTextSettingsBody() {
    ...
    return `
      <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:10px">
        <div style="font-size:11px;color:var(--text-soft);line-height:1.4">Setting ini hanya muncul di admin. File printer staff otomatis baca dari Firestore saat refresh/login.</div>
  ...
  async function saveReceiptTextSettings() {
    ...
    try {
      ...
      console.error(e);
      showToast('Gagal simpan setting struk: ' + (e.code || e.message || 'cek Firestore'), true);
    }
    setLoading(false);
  }

  function renderStaffDailyHomeNotePublicCard() {
    if (!currentUser || isAdmin() || cachedStaffDailyHomeNote?.enabled === false) return '';
    const note = getStaffDailyHomeNoteText();
    const updatedAtMs = Number(cachedStaffDailyHomeNote?.updatedAtMs || 0);
    const updatedText = updatedAtMs ? new Date(updatedAtMs).toLocaleString('id-ID', { dateStyle: 'medium', timeStyle: 'short' }) : 'Catatan aktif';
    return `
      <div class="card staff-daily-public-card page-in s1">
        <div class="staff-daily-public-inner">
          <div class="staff-daily-status-head">
            <div class="staff-daily-avatar"><i class="fas fa-bullhorn"></i></div>
            <div style="min-width:0;flex:1">
              <div class="staff-daily-status-name">Catatan Home</div>
              <div class="staff-daily-status-meta">Untuk Staff & Harian · ${escapeHtml(updatedText)}</div>
            </div>
            <span class="status-pill status-ok" style="font-size:9px;padding:4px 7px">Aktif</span>
          </div>
          <div class="staff-daily-public-text">${renderStaffDailyNoteLinks(note)}</div>
        </div>
      </div>`;
  }

  function renderStaffDailyHomeNoteControlBody() {
    const note = getStaffDailyHomeNoteText();
    const enabled = cachedStaffDailyHomeNote?.enabled !== false;
    const updatedBy = cachedStaffDailyHomeNote?.updatedByName || cachedStaffDailyHomeNote?.updatedBy || '-';
    const updatedAtMs = Number(cachedStaffDailyHomeNote?.updatedAtMs || 0);
    const updatedText = updatedAtMs ? new Date(updatedAtMs).toLocaleString('id-ID', { dateStyle: 'medium', timeStyle: 'short' }) : 'Default aplikasi';
    const countText = `${String(note || '').length}/320`;
    return `
      <div class="staff-daily-status-editor">
        <div class="staff-daily-status-head">
          <div class="staff-daily-avatar"><i class="fas fa-note-sticky"></i></div>
          <div style="min-width:0;flex:1">
            <div class="staff-daily-status-name">Catatan Home Staff & Harian</div>
            <div class="staff-daily-status-meta">Update: ${escapeHtml(updatedText)} · ${escapeHtml(updatedBy)}</div>
          </div>
          <label class="staff-daily-status-toggle" title="Aktif / nonaktif card catatan di home staff dan harian">
            <input id="staff-daily-note-enabled" type="checkbox" ${enabled ? 'checked' : ''} onchange="updateStaffDailyNoteLivePreview()">
            <span>Aktif</span>
          </label>
        </div>
        <div class="staff-daily-composer">
          <textarea id="staff-daily-note-input" maxlength="320" rows="4" placeholder="Tulis catatan atau tempel link update/download untuk staff dan karyawan harian..." oninput="updateStaffDailyNoteLivePreview()">${escapeHtml(note)}</textarea>
          <div class="staff-daily-composer-foot">
            <span style="font-size:10.5px;color:var(--text-soft);line-height:1.35"><i class="fas fa-house"></i> Tampil di halaman utama staff & harian · link otomatis bisa diklik</span>
            <span id="staff-daily-note-count" class="staff-daily-note-count">${escapeHtml(countText)}</span>
          </div>
        </div>
        <div id="staff-daily-note-live-preview" class="staff-daily-preview-wrap ${enabled ? '' : 'off'}">
          <div class="staff-daily-preview-title"><i class="fas ${enabled ? 'fa-eye' : 'fa-eye-slash'}"></i> Preview Home</div>
          <div class="staff-daily-preview-text">${enabled ? renderStaffDailyNoteLinks(note) : 'Card catatan sedang nonaktif dan tidak tampil di halaman utama.'}</div>
        </div>
        <div class="staff-daily-actions">
          <span style="font-size:10.5px;color:var(--text-soft);line-height:1.35">Logika simpan, default, dan on/off tetap sama.</span>
          <div style="display:flex;gap:7px;flex-wrap:wrap">
            <button onclick="resetStaffDailyHomeNoteInput()" class="btn btn-glass"><i class="fas fa-rotate-left"></i> Default</button>
            <button onclick="saveStaffDailyHomeNote()" class="btn btn-primary"><i class="fas fa-paper-plane"></i> Update Status</button>
          </div>
        </div>
      </div>`;
  }

  function renderStaffDailyHomeNoteHomeCard() {
    if (!isAdmin()) return '';
    return `<div class="card staff-daily-status-card page-in s4">${renderStaffDailyHomeNoteControlBody()}</div>`;
  }

  function updateStaffDailyNoteLivePreview() {
    const input = $('staff-daily-note-input');
    const check = $('staff-daily-note-enabled');
    const preview = $('staff-daily-note-live-preview');
    const count = $('staff-daily-note-count');
    const value = String(input?.value || '');
    const note = value.trim();
    const enabled = check?.checked !== false;
    if (count) count.textContent = `${value.length}/320`;
    if (preview) {
      preview.classList.toggle('off', !enabled);
      preview.innerHTML = enabled
        ? `<div class="staff-daily-preview-title"><i class="fas fa-eye"></i> Preview Home</div><div class="staff-daily-preview-text">${renderStaffDailyNoteLinks(note || DEFAULT_STAFF_DAILY_NOTE)}</div>`
        : `<div class="staff-daily-preview-title"><i class="fas fa-eye-slash"></i> Preview Home</div><div class="staff-daily-preview-text">Card catatan sedang nonaktif dan tidak tampil di halaman utama.</div>`;
    }
  }

  async function saveStaffDailyHomeNote() {
    if (!isAdmin()) return showToast('Hanya admin!', true);
    if (isBusy) return showToast('Tunggu proses selesai', true);
    const note = String($('staff-daily-note-input')?.value || '').trim();
    const enabled = $('staff-daily-note-enabled')?.checked !== false;
    if (enabled && !note) return showToast('Catatan tidak boleh kosong saat aktif', true);
    if (note.length > 320) return showToast('Catatan maksimal 320 karakter', true);
    const pin = await askPin('Simpan catatan Home Staff & Harian?\nMasukkan PIN admin:');
    if (!pin) return;
    if (String(pin) !== String(currentUser?.pin || '')) return showToast('PIN admin salah!', true);
    setLoading(true);
    try {
      ...
      await setDoc(doc(db, 'closings', STAFF_DAILY_NOTE_DOC_ID), payload, { merge: true });
      cachedStaffDailyHomeNote = { ...cachedStaffDailyHomeNote, ...payload };
      await logAudit('staff_daily_home_note_update', { enabled, note: payload.note });
      showToast('✓ Catatan home staff/harian disimpan');
      await refreshCurrentPage();
    } catch (e) {
      console.error(e);
      showToast('Gagal simpan catatan home staff/harian', true);
    }
    setLoading(false);
  }

  function resetStaffDailyHomeNoteInput() {
    const input = $('staff-daily-note-input');
    if (input) input.value = DEFAULT_STAFF_DAILY_NOTE;
    const check = $('staff-daily-note-enabled');
    if (check) check.checked = true;
    updateStaffDailyNoteLivePreview();
  }
  ...
  async function handleClockIn() {
    if (isBusy || clockInInFlight) return;
    if (!currentUser) return;
    if (hasClockedInToday()) return showToast('Sudah absen hari ini!');
    ...
    setLoading(true);
    try {
      const existing = await findExistingAttendanceToday(user, todayKey);
      if (existing) {
        const createdAtMs = getRecordTimeMs(existing) || Number(existing.createdAtMs || Date.now());
        saveAttendanceCache(user, { dateKey: todayKey, createdAtMs });
        upsertAttendanceRecord({ ...existing, user, name: existing.name || currentUser.name, dateKey: todayKey, createdAtMs });
        attendanceReady = true;
        refreshCurrentPage();
      ...
      const lockedDocId = getAttendanceDocId(user, todayKey);
      ...
      saveAttendanceCache(user, { dateKey: todayKey, createdAtMs: nowMs });
      upsertAttendanceRecord(optimisticRecord);
      attendanceReady = true;
      refreshCurrentPage();

      await setDoc(doc(db, 'attendance', lockedDocId), {
        user,
        name: currentUser.name,
        dateKey: todayKey,
        createdAt: serverTimestamp(),
  ...
  async function openTransactionModal() {
    ...
    setTimeout(() => $('modal-pos-note').focus(), 100);
  }
  ...
  function openTransactionPaymentModal(draft) {
    ...
    if (box) {
      box.innerHTML = `<p class="label-xs" style="margin-bottom:6px">Konfirmasi transaksi</p><p style="font-family:var(--num-font);font-size:24px;font-weight:800;color:var(--accent);line-height:1">Rp ${formatRupiah(pendingTransactionPaymentDraft.amount || 0)}</p><p style="font-size:12px;color:var(--text);font-weight:700;margin-top:8px;white-space:pre-wrap;word-break:break-word">${escapeHtml(pendingTransactionPaymentDraft.note || 'Transaksi')}</p><p style="font-size:11px;color:var(--text-soft);margin-top:8px">Transaksi belum dikirim ke Firebase. Pilih salah satu metode untuk menyimpan sebagai sukses.</p>`;
    }
  ...
  function prepareTransactionPaymentFromModal() {
    if (isBusy) return;
    const amount = parseInt($('modal-pos-amount')?.getAttribute('data-numeric') || '0', 10);
    const note = String($('modal-pos-note')?.value || '').trim() || 'Transaksi';
    if (!amount || amount <= 0) return showToast('Nominal tidak valid!', true);
    openTransactionPaymentModal({ amount, note });
  }
  ...
  async function handleTransactionFromModal(paymentMethod = '') {
    ...
    const note = String(draft.note || $('modal-pos-note')?.value || '').trim() || 'Transaksi';
    if (!amount || amount <= 0) return showToast('Nominal tidak valid!', true);
    ...
    setLoading(true);
    try {
      const latest = await fetchLatestBonusBeforeTransaction(currentUser.username);
      const freshUser = latest.user || currentUser;
      ...
      const userRole = normalizeRole(freshUser.role || currentUser.role || 'staff');
      const bonusGroup = userRole === 'harian' ? 'harian' : 'staff';
      const bonusRate = userBonus.transactionBonusRate;
      await addDoc(collection(db, 'transactions'), {
        user: sanitizeUsername(freshUser.username || currentUser.username),
      ...
      playTransactionSuccessSound();
      showToast(`✓ Transaksi disimpan · ${paymentInfo.label}`);
      closeTransactionPaymentModal(true);
      closeTransactionModal();
    } catch (e) {
      console.error(e);
  ...
  async function updateHomeStatus() {
    ...
    if (isAdmin()) {
      const userCount = cachedUsers.length || new Set(cachedTransactions.map(t => t.user)).size || 0;
      const activeToday = new Set(cachedAttendance.filter(a => !isRecordDeleted(a) && extractDateKey(a) === toDateKey(nowDate())).map(a => sanitizeUsername(a.user))).size;
      el.innerHTML = `<div style="display:flex;flex-direction:column;gap:3px"><div style="display:flex;align-items:center;gap:8px;font-size:13px;font-weight:700;color:var(--accent)"><i class="fas fa-shield-halved"></i> Panel admin aktif</div><div style="font-size:12px;color:var(--text-soft)">Total user: ${userCount} · aktif hari ini: ${activeToday}</div><div style="font-size:11px;color:var(--text-ghost)">Admin bisa transaksi langsung tanpa absen.</div></div>`;
      return;
    }
    if (isTransactionsClosedToday()) {
      const closing = getTodayUserClosing(currentUser?.username) || getTodayGlobalClosing();
      const jamClosing = getClosingDisplayTime(closing) || '--:--';
      el.innerHTML = `<div style="display:flex;flex-direction:column;gap:3px"><div style="display:flex;align-items:center;gap:8px;font-size:13px;font-weight:700;color:var(--amber)"><i class="fas fa-lock"></i> Transaksi sudah closing</div><div style="font-size:12px;color:var(--text-soft)">Jam closing: ${jamClosing} WIB · terbuka lagi besok.</div></div>`;
    } else if (isDailyEmployee(currentUser)) {
      el.innerHTML = `<div style="display:flex;flex-direction:column;gap:3px"><div style="display:flex;align-items:center;gap:8px;font-size:13px;font-weight:800;color:var(--accent)"><i class="fas fa-user-check"></i> Karyawan Harian</div><div style="font-size:12px;color:var(--text-soft)">Tanpa absen · transaksi hari ini saja.</div></div>`;
    } else if (!attendanceReady) {
      if (hasClockedInToday()) {
        const rec = getTodayAttendanceRecord();
        const jam = formatAttendanceTime(rec);
        el.innerHTML = `<div style="display:flex;flex-direction:column;gap:3px"><div style="display:flex;align-items:center;gap:8px;font-size:13px;font-weight:700;color:var(--green)"><i class="fas fa-circle-check"></i> Status absen tersimpan</div><div style="font-size:12px;color:var(--text-soft)">Jam absen: ${jam} · memverifikasi ke server...</div></div>`;
      } else {
        el.innerHTML = `<div style="display:flex;align-items:center;gap:8px;font-size:12px;color:var(--text-soft)"><i class="fas fa-circle-notch fa-spin"></i> Memuat status absen...</div>`;
      }
    } else if (hasClockedInToday()) {
      const rec = getTodayAttendanceRecord();
      const jam = formatAttendanceTime(rec);
      el.innerHTML = `<div style="display:flex;flex-direction:column;gap:3px"><div style="display:flex;align-items:center;gap:8px;font-size:13px;font-weight:700;color:var(--green)"><i class="fas fa-circle-check"></i> Sudah absen hari ini</div><div style="font-size:12px;color:var(--text-soft)">Jam absen: ${jam}</div></div>`;
    } else {
      ...
      el.innerHTML = `<button onclick="handleClockIn()" ${disabled?'disabled':''} class="btn btn-primary" style="padding:12px 16px;width:auto;border-radius:12px"><i class="fas fa-fingerprint" style="font-size:13px"></i> Absen</button>`;
    }
  }


  function renderStaffTodayHistory() {
    const box = $('staff-today-history');
    if (!box) return;
    const today = toDateKey(nowDate());
    const items = sortByCreatedDesc(getVisibleTransactions().filter(t => getTransactionDateKey(t) === today));
    const total = items.reduce((a,b)=>a+Number(b.amount||0),0);
    box.innerHTML = `
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">
        <div><p class="label-xs" style="margin-bottom:2px">Riwayat Hari Ini</p><p style="font-size:11px;color:var(--text-soft)">${items.length} transaksi · Rp ${formatRupiah(total)}</p></div>
        <button onclick="openTransactionModal()" class="btn btn-primary" style="padding:9px 11px;font-size:11.5px;border-radius:12px"><i class="fas fa-plus"></i> Transaksi</button>
      </div>
      <div style="display:flex;flex-direction:column;gap:8px">
        ${items.length ? items.slice(0, 8).map(t => `
          <div class="trx-card" style="padding:10px 12px;border-radius:13px">
            <div style="min-width:0;flex:1">
              <div style="display:flex;align-items:center;justify-content:space-between;gap:10px">
                <p class="trx-note" style="max-width:185px;margin:0;font-size:12px;color:var(--text);font-weight:700">${escapeHtml(t.note || 'Transaksi')}</p>
                <p class="trx-amount" style="font-size:14px;white-space:nowrap">Rp ${formatRupiah(t.amount)}</p>
              </div>
              <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-top:4px">
                <span class="trx-time" style="font-size:9.5px">${dateToTimeInput(new Date(getRecordTimeMs(t) || Date.now()))} · ${getTransactionPaymentLabel(t)}</span>
                <button onclick="deleteTransaction('${t.id}')" class="btn btn-danger" style="padding:5px 8px;font-size:10px;border-radius:9px"><i class="fas fa-trash"></i></button>
              </div>
            </div>
          </div>`).join('') : `<div style="padding:18px 10px;text-align:center;border:1px dashed var(--border);border-radius:14px;color:var(--text-soft);font-size:12px">Belum ada transaksi hari ini.</div>`}
      </div>`;
  }


  function renderStaffTodayHistoryPage() {
    const box = $('staff-today-history-page');
    if (!box) return;
    const today = toDateKey(nowDate());
    const items = sortByCreatedDesc(getVisibleTransactions().filter(t => getTransactionDateKey(t) === today));
    const total = items.reduce((a,b)=>a+Number(b.amount||0),0);
    box.innerHTML = `
      <div class="card page-in s1" style="padding:14px 15px;margin-bottom:12px;background:linear-gradient(145deg,var(--bg3),var(--bg2));border-color:rgba(91,143,255,.16)">
        <div style="display:flex;align-items:center;justify-content:space-between;gap:10px">
          <div>
            <p class="label-xs" style="margin-bottom:5px">Total Hari Ini</p>
            <p style="font-family:var(--num-font);font-size:25px;font-weight:850;color:var(--accent);line-height:1">Rp ${formatRupiah(total)}</p>
            <p style="font-size:11px;color:var(--text-soft);margin-top:6px">${items.length} transaksi tersimpan · ${formatDateLabel(today)}</p>
          </div>
          <button onclick="openTransactionModal()" class="btn btn-primary" style="width:46px;height:46px;border-radius:16px;padding:0;font-size:18px"><i class="fas fa-plus"></i></button>
        </div>
      </div>
      <div style="display:flex;flex-direction:column;gap:9px;padding-bottom:86px">
        ${items.length ? items.map(t => `
          <div class="trx-card" style="padding:12px 13px;border-radius:14px">
            <div class="trx-icon"><i class="fas fa-receipt"></i></div>
            <div style="min-width:0;flex:1">
              <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:10px">
                <div style="min-width:0;flex:1">
                  <p class="trx-note" style="max-width:190px;margin:0;font-size:13.5px;color:var(--text);font-weight:800;white-space:normal;line-height:1.3">${escapeHtml(t.note || 'Transaksi')}</p>
                  <p class="trx-time" style="font-size:10px;margin-top:5px">${dateToTimeInput(new Date(getRecordTimeMs(t) || Date.now()))} · ${getTransactionPaymentLabel(t)}</p>
                </div>
                <div style="text-align:right;flex-shrink:0">
                  <p class="trx-amount" style="font-size:15px;white-space:nowrap;color:var(--green)">Rp ${formatRupiah(t.amount)}</p>
                  <button onclick="deleteTransaction('${t.id}')" class="btn btn-danger" style="margin-top:7px;padding:5px 8px;font-size:10px;border-radius:9px"><i class="fas fa-trash"></i></button>
                </div>
              </div>
            </div>
          </div>`).join('') : `<div class="card" style="padding:28px 16px;text-align:center;color:var(--text-soft);font-size:12px;border-style:dashed"><i class="fas fa-receipt" style="font-size:22px;margin-bottom:10px;color:var(--text-ghost)"></i><br>Belum ada transaksi hari ini.</div>`}
      </div>`;
  }
  ...
  function renderClosingHistoryCard() {
    ...
    // Data closing tetap tersimpan di Firebase untuk laporan, bonus, anti double closing, dan audit.
    return '';
  }
  ...
  function renderRismaManualClosingPanel() {
    ...
    }).join('') : `<div style="font-size:12px;color:var(--text-soft);padding:12px;border:1px dashed var(--border);border-radius:12px;text-align:center">Belum ada user yang sudah absen atau karyawan harian aktif.</div>`;
    return `<div style="margin-top:10px;background:linear-gradient(145deg,var(--bg3),var(--bg2));border:1px solid rgba(190,24,93,.22);border-radius:18px;padding:14px;box-shadow:0 10px 28px rgba(0,0,0,.12);position:relative;overflow:hidden">
      ...
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:10px;position:relative;z-index:1;margin-bottom:10px">
        <div style="min-width:0;flex:1">
          ...
          <div style="font-size:11px;color:var(--text-soft);margin-top:5px;line-height:1.35">Admin pilih user yang boleh diclosing oleh Risma. Yang muncul hanya staff yang sudah absen hari ini dan karyawan harian.</div>
        </div>
      ...
      <div style="font-size:10px;color:var(--text-soft);line-height:1.35;margin-top:8px;position:relative;z-index:1">Catatan: ini tidak langsung closing. Staff yang tidak absen tidak masuk target, karena tidak hadir memang tidak perlu diclosing.</div>
    </div>`;
  }
  ...
  function renderAdminClosingPanel() {
    ...
    const mainDesc = closing
      ...
      : `Pilih user yang sudah absen hari ini atau karyawan harian. Yang tidak hadir tidak perlu diclosing.`;
    box.innerHTML = `
  ...
  // Cash Fisik = Omset Firebase/Kasir hari ini - Operasional Toko - QRIS - Tabungan.
  ...
  function getFirebaseOmzetForCashFisik(dateKey = toDateKey(nowDate())) {
  ...
  async function getHomeCashFisikBreakdown(dateKey = toDateKey(nowDate()), force = false) {
    ...
    const fbTotal = getFirebaseOmzetForCashFisik(dateKey);
  ...
  async function updateHomeCashFisikCard(dateKey = toDateKey(nowDate()), force = false) {
    ...
    if (infoEl) {
      ...
      // Breakdown Firebase/Ops/QRIS/Tabungan tetap dihitung di belakang, tapi tidak ditampilkan.
      infoEl.innerText = '';
  ...
  async function updateHomeStats() {
    ...
    // HOME/Beranda admin harus selalu GLOBAL.
    // Filter user yang dipilih di halaman Global/Riwayat tidak boleh bocor ke kartu Home.
    // Untuk staff, getVisibleTransactions()/getVisibleAttendance() sudah otomatis dibatasi ke data sendiri.
    const sourceTransactions = visibleTransactions;
    const sourceAttendance = visibleAttendance;
    ...
    const bonusScopeUser = isAdmin() ? null : currentUser.username;
    const bonusBreakdown = getBonusBreakdown(sourceTransactions, bonusScopeUser, month);
    const todayBonusBreakdown = getBonusBreakdown(sourceTransactions, bonusScopeUser, { type: 'daily', dateKey: today });
    const dailyMode = !isAdmin() && isDailyEmployee(currentUser);
    const dailyPay = dailyMode ? getDailyEmployeeTodaySalary(currentUser.username, today) : null;
    const dailyManualBonus = dailyMode ? getMonthlyManualBonus(currentUser.username, month) : 0;
    const rec = sortByCreatedDesc(sourceAttendance.filter(a => extractDateKey(a) === today))[0] || null;
    ...
    if ($('total-bonus')) $('total-bonus').innerText = dailyMode ? `Rp ${formatRupiah(dailyPay.salary + dailyManualBonus)}` : `Rp ${formatRupiah(bonusBreakdown.totalBonus)}`;
    if ($('bonus-text')) {
      if (dailyMode) {
        $('bonus-text').style.display = '';
        $('bonus-text').innerText = `${dailyPay.items.length} transaksi hari ini${dailyManualBonus ? ' · bonus manual Rp ' + formatRupiah(dailyManualBonus) : ''}`;
      } else {
        $('bonus-text').style.display = '';
    ...
    const todayClosing = isAdmin() ? getTodayGlobalClosing() : (getTodayUserClosing(currentUser?.username) || getTodayGlobalClosing());
    const closingJam = getClosingDisplayTime(todayClosing);
    if ($('today-attendance-info')) $('today-attendance-info').innerText = dailyMode ? 'Harian' : (todayClosing ? (closingJam || '--:--') : (rec ? formatAttendanceTime(rec).slice(0,5) : '--:--'));
    const card = $('today-attendance-card'), lbl = $('today-attendance-label'), info = $('today-attendance-info');
    if (card && lbl && info) {
      if (dailyMode) {
        card.style.background = 'var(--accent-dim)'; card.style.borderColor = 'rgba(91,143,255,0.22)';
        lbl.style.color = 'var(--accent)'; info.style.color = 'var(--accent)';
        lbl.innerText = 'Mode';
      } else if (todayClosing) {
        card.style.background = 'var(--amber-dim)'; card.style.borderColor = 'rgba(255,196,107,0.22)';
        lbl.style.color = 'var(--amber)'; info.style.color = 'var(--amber)';
        lbl.innerText = 'Jam Closing';
      } else if (rec) {
        card.style.background = 'var(--green-dim)'; card.style.borderColor = 'rgba(82,216,158,0.2)';
        lbl.style.color = 'var(--green)'; info.style.color = 'var(--green)';
        lbl.innerText = 'Jam';
      } else {
        card.style.background = 'var(--red-dim)'; card.style.borderColor = 'rgba(255,107,107,0.2)';
        lbl.style.color = 'var(--red)'; info.style.color = 'var(--red)';
        lbl.innerText = 'Jam';
      }
    }
    if ($('scope-chip')) $('scope-chip').textContent = isAdmin() ? 'Semua User' : 'Data Saya';
    if ($('staff-closing-status')) $('staff-closing-status').innerHTML = renderClosingStatusHtml(true);
    if (isAdmin()) renderAdminClosingPanelMaybe();
    if (!isAdmin()) {
      renderStaffTodayHistory();
      // Render card rata-rata absen bulanan
      const perfCard = $('attendance-performance-card');
      if (perfCard) {
        if (dailyMode) {
          perfCard.innerHTML = `
            <div style="display:flex;align-items:center;justify-content:space-between;gap:10px">
              <div style="min-width:0;flex:1">
                <p class="label-xs" style="margin-bottom:4px">Gaji Karyawan Harian</p>
                <p style="font-size:12px;color:var(--text-soft);line-height:1.35">Hanya data hari ini. Transaksi tetap masuk ke total global admin.</p>
                <p style="font-size:10.5px;color:var(--text-ghost);margin-top:4px">Omzet: Rp ${formatRupiah(dailyPay.income)} · ${dailyPay.items.length} transaksi</p>
              </div>
              <div style="text-align:center;border:1px solid rgba(255,196,107,0.2);border-radius:11px;padding:8px 12px;background:var(--amber-dim);flex-shrink:0">
                <p style="font-family:var(--num-font);font-size:20px;font-weight:850;color:var(--amber);line-height:1">Rp ${formatRupiah(dailyPay.salary)}</p>
                <p style="font-size:9px;font-weight:800;letter-spacing:.05em;text-transform:uppercase;color:var(--amber);margin-top:3px">Gaji Hari Ini</p>
              </div>
            </div>`;
          return;
        }
        const month = toMonthKey(nowDate());
        const perf = getMonthlyAttendancePerformance(currentUser.username);
        if (!perf) {
          perfCard.innerHTML = `
            <div style="display:flex;align-items:center;justify-content:space-between;gap:10px">
              <div>
                <p class="label-xs" style="margin-bottom:4px">Rata-rata Jam Absen</p>
                <p style="font-size:11px;color:var(--text-ghost)">Bulan ${month} · belum ada data</p>
              </div>
              <div style="text-align:center;border:1px solid var(--border);border-radius:11px;padding:8px 12px;background:var(--surface)">
                <p style="font-family:var(--num-font);font-size:17px;font-weight:800;color:var(--text-ghost);line-height:1">--:--</p>
                <p style="font-size:9px;font-weight:700;letter-spacing:.05em;text-transform:uppercase;color:var(--text-ghost);margin-top:3px">Belum Ada Data</p>
              </div>
            </div>`;
        } else {
          const color = perf.isGood ? 'var(--green)' : 'var(--amber)';
          const bgColor = perf.isGood ? 'var(--green-dim)' : 'var(--amber-dim)';
          const borderColor = perf.isGood ? 'rgba(82,216,158,0.2)' : 'rgba(255,196,107,0.2)';
          const icon = perf.isGood ? 'fa-circle-check' : 'fa-circle-exclamation';
          const label = perf.isGood ? 'Performa Baik ✓' : 'Perlu Ditingkatkan';
          perfCard.innerHTML = `
            <div style="display:flex;align-items:center;justify-content:space-between;gap:10px">
              <div style="flex:1;min-width:0">
                ...
                <div style="display:flex;align-items:center;gap:6px;margin-top:2px">
                  ...
                  <p style="font-size:12px;font-weight:700;color:${color}">${label}</p>
                </div>
                <p style="font-size:10px;color:var(--text-ghost);margin-top:3px">Bulan ${month} · ${perf.count} hari hadir</p>
              </div>
              <div style="text-align:center;border:1px solid ${borderColor};border-radius:11px;padding:8px 12px;background:${bgColor};flex-shrink:0">
                <p style="font-family:var(--num-font);font-size:20px;font-weight:800;color:${color};line-height:1">${perf.avgLabel}</p>
                <p style="font-size:9px;font-weight:700;letter-spacing:.05em;text-transform:uppercase;color:${color};margin-top:3px;opacity:.8">Rata-rata</p>
              </div>
            </div>`;
        }
      }
  ...
  async function renderHistory() {
    ...
    // Hitung summary tambahan
    const opsItems = visibleItems.filter(t => String(t.note||'').startsWith('[OPS]'));
    const incomeItems = visibleItems.filter(t => !String(t.note||'').startsWith('[OPS]'));
    const totalIncome = incomeItems.reduce((a,b)=>a+Number(b.amount||0),0);
    const totalOps = opsItems.reduce((a,b)=>a+Number(b.amount||0),0);

    // Group by tanggal
    const groups = {};
    visibleItems.forEach(t => {
      const dk = getTransactionDateKey(t) || 'lainnya';
    ...
    list.innerHTML = `
      <!-- HERO SUMMARY -->
      <div class="stat-hero page-in" style="margin-bottom:10px">
        <p class="stat-label">${isAdmin() ? 'Total Transaksi Global' : 'Total Transaksi'}</p>
        <p class="stat-value">Rp ${formatRupiah(total)}</p>
        <p class="stat-sub">${visibleItems.length} dari ${filtered.length} transaksi · ${isAdmin() ? getSelectedUserLabel() : 'akun saya'}</p>
      </div>

      <!-- SUMMARY BAR -->
      <div class="rh-summary-bar page-in s1">
        <div class="rh-summary-item">
          <div class="rh-summary-val green">Rp ${formatRupiah(totalIncome)}</div>
          <div class="rh-summary-lbl">Pemasukan</div>
        </div>
        ${totalOps > 0 ? `<div class="rh-summary-item">
          <div class="rh-summary-val red">Rp ${formatRupiah(totalOps)}</div>
          <div class="rh-summary-lbl">Operasional</div>
        </div>` : ''}
        <div class="rh-summary-item">
          <div class="rh-summary-val accent">${sortedDates.length}</div>
          <div class="rh-summary-lbl">Hari Aktif</div>
        </div>
      </div>
      ...
      <!-- SEARCH TEKS -->
      <div class="rh-search-wrap page-in s1">
        <i class="fas fa-magnifying-glass"></i>
        <input type="text" id="history-text-search" class="g-input rh-search-input" placeholder="Cari catatan transaksi..." oninput="window.__rhSearchFilter(this.value)" autocomplete="off">
      </div>

      <!-- FILTER TANGGAL RANGE -->
      <div style="display:flex;gap:7px;margin-bottom:8px" class="page-in s1">
      ...
      <div style="display:flex;gap:7px;margin-bottom:11px" class="page-in s2">
        <button onclick="filterHistoryByDateRange()" class="btn btn-primary" style="flex:1;padding:9px;font-size:12px"><i class="fas fa-magnifying-glass"></i> Cari</button>
        <button onclick="resetHistoryFilter()" class="btn btn-glass" style="flex:1;padding:9px;font-size:12px"><i class="fas fa-rotate"></i> Reset</button>
      </div>

      <!-- TAB FILTER WAKTU -->
      <div class="tab-group page-in s2" style="margin-bottom:13px">
      ...
      <div id="rh-grouped-list" class="page-in s3">
        ${sortedDates.length === 0 ? `<div style="text-align:center;padding:56px 0;color:var(--text-ghost)"><i class="fas fa-inbox" style="font-size:30px;margin-bottom:10px;display:block;opacity:0.5"></i><p style="font-size:12.5px">Tidak ada transaksi</p></div>` :
          sortedDates.map(dk => {
            const dayItems = groups[dk];
            const dayTotal = dayItems.reduce((a,b)=>a+Number(b.amount||0),0);
            return `
              <div class="rh-date-group">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px">
                  <span class="rh-date-label"><i class="fas fa-calendar-day" style="font-size:8px"></i>${formatDateLabel(dk)}</span>
                  <span style="font-size:10px;font-weight:800;color:var(--accent);font-family:var(--num-font)">Rp ${formatRupiah(dayTotal)} · ${dayItems.length} trx</span>
                </div>
                <div style="display:flex;flex-direction:column;gap:5px">
                  ${dayItems.map(t => {
                    const isOps = String(t.note||'').startsWith('[OPS]');
                    const ms = getRecordTimeMs(t);
                    const timeStr = ms ? dateToTimeInput(new Date(ms)) : '';
                    const noteClean = isOps ? String(t.note||'').replace(/^\[OPS\]\s*/,'') : (t.note || 'Transaksi');
                    return `<div class="trx-card-v2">
                      <div class="trx-icon-v2 ${isOps ? 'trx-icon-ops' : 'trx-icon-income'}">
                        <i class="fas ${isOps ? 'fa-arrow-trend-down' : 'fa-arrow-trend-up'}"></i>
                      </div>
                      <div class="trx-main">
                        <div class="trx-top-row">
                          <span class="trx-amount-v2 ${isOps ? 'trx-amount-ops' : 'trx-amount-income'}">${isOps ? '−' : ''}Rp ${formatRupiah(t.amount)}</span>
                          <span class="${isOps ? 'trx-badge-ops' : 'trx-badge-pos'}">${isOps ? 'OPS' : 'POS'}</span>
                          ${isOps ? '' : getTransactionPaymentBadge(t)}
                        </div>
                        <p class="trx-note-v2">${escapeHtml(noteClean)}</p>
                        <div class="trx-meta-row">
                          ${timeStr ? `<span class="trx-time-v2"><i class="fas fa-clock" style="font-size:8px;margin-right:2px"></i>${timeStr} · ${getTransactionPaymentLabel(t)}</span>` : `<span class="trx-time-v2">${getTransactionPaymentLabel(t)}</span>`}
                          ${isAdmin() ? `<span class="trx-user-chip"><i class="fas fa-user" style="font-size:8px"></i>${escapeHtml(t.name || t.user)}</span>` : ''}
                        </div>
                      </div>
                      <button onclick="deleteTransaction('${t.id}')" class="trx-del-btn" title="Hapus">
      ...
      ${hasMore ? `<button onclick="increaseHistoryDisplayLimit()" class="btn btn-glass page-in s4" style="width:100%;margin-top:12px;padding:11px 12px;font-size:12px"><i class="fas fa-chevron-down"></i> Muat ${Math.min(HISTORY_PAGE_STEP, filtered.length - visibleItems.length)} transaksi lagi</button>` : ''}
    `;
    ...
    // Live search filter (client-side, tidak ubah state JS)
    window.__rhSearchFilter = function(q) {
      const term = (q || '').toLowerCase().trim();
      const allGroups = document.querySelectorAll('#rh-grouped-list .rh-date-group');
      allGroups.forEach(grp => {
        const cards = grp.querySelectorAll('.trx-card-v2');
  ...
  function renderMonthlyReport() {
    ...
    const warning = transactions.length >= 9900 ? `<div class="card page-in s1" style="padding:11px 12px;border-color:rgba(255,196,107,.24);background:var(--amber-dim);color:var(--amber);font-size:11px;line-height:1.35;margin-bottom:10px"><i class="fas fa-triangle-exclamation"></i> Data bulan ini mendekati batas sinkron 10.000 baris. Pakai backup Excel untuk arsip lengkap.</div>` : '';

    const dailyFiltered = dailyRows.filter(d => d.trxCount || d.hadirCount || d.closingCount);
    ...
    return `
      ...
      <div class="card page-in s1" style="padding:12px 13px;margin-bottom:10px">
        <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:10px">
          <div>
            <p class="label-xs" style="margin:0;color:var(--accent)">Periode Laporan</p>
            <p style="font-size:11px;color:var(--text-soft);margin-top:2px">${escapeHtml(targetLabel)}</p>
          </div>
          <span class="status-pill status-ok"><i class="fas fa-calendar-days"></i> ${escapeHtml(meta.label)}</span>
        </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:7px">
          <button onclick="syncMonthlyReportData()" class="btn btn-glass" style="padding:10px 8px;font-size:11px"><i class="fas fa-rotate"></i> Sinkron</button>
      ...
      <!-- TABS NAVIGASI SECTION -->
      <div class="report-section-tabs page-in s1">
        <button class="report-stab active" onclick="window.__reportTab(this,'rep-overview')"><i class="fas fa-chart-pie"></i> Ringkasan</button>
        <button class="report-stab" onclick="window.__reportTab(this,'rep-users')"><i class="fas fa-users"></i> Per User</button>
        <button class="report-stab" onclick="window.__reportTab(this,'rep-daily')"><i class="fas fa-calendar-days"></i> Harian</button>
        <button class="report-stab" onclick="window.__reportTab(this,'rep-detail')"><i class="fas fa-list-ul"></i> Detail</button>
      </div>

      <!-- TAB: RINGKASAN -->
      <div id="rep-overview" class="report-section active">
        <div class="stat-hero page-in" style="margin-bottom:10px">
          <p class="stat-label">Total Omzet · ${escapeHtml(meta.label)}</p>
          <p class="stat-value">Rp ${formatRupiah(totals.omzet)}</p>
          <p class="stat-sub">${totals.trxCount} transaksi · ${totals.daysWithSales} hari jualan · rata-rata Rp ${formatRupiah(totals.avgPerSalesDay)}/hari</p>
        </div>
        <div class="report-kpi-row page-in s1">
          <div class="report-kpi-card">
            <div class="report-kpi-label">User Aktif</div>
            <div class="report-kpi-value" style="color:var(--green)">${totals.activeUserCount}</div>
            <div class="report-kpi-sub">user bertransaksi</div>
          </div>
          <div class="report-kpi-card">
            <div class="report-kpi-label">Absensi</div>
            <div class="report-kpi-value" style="color:var(--accent)">${totals.uniqueAttendance}</div>
            <div class="report-kpi-sub">user-hari hadir</div>
          </div>
          <div class="report-kpi-card">
            <div class="report-kpi-label">Rata-rata/Trx</div>
            <div class="report-kpi-value" style="font-size:15px">Rp ${formatRupiah(totals.avgPerTransaction)}</div>
            <div class="report-kpi-sub">${totals.trxCount} trx dihitung</div>
          </div>
          <div class="report-kpi-card">
            <div class="report-kpi-label">Closing</div>
            <div class="report-kpi-value" style="color:var(--amber)">${totals.closingCount}</div>
            <div class="report-kpi-sub">Rp ${formatRupiah(totals.closingBonus)}</div>
          </div>
        </div>

        <!-- Highlight -->
        <div class="card page-in s2" style="padding:12px 13px;margin-bottom:10px">
          <p class="label-xs" style="margin-bottom:9px">🏆 Highlight Bulan Ini</p>
          <div style="display:flex;flex-direction:column;gap:6px">
            ${topUser ? `<div class="bonus-row"><span class="bonus-row-label"><i class="fas fa-star" style="color:var(--green);margin-right:5px"></i>Top User: ${escapeHtml(topUser.name)}</span><span class="bonus-row-val" style="color:var(--green)">Rp ${formatRupiah(topUser.omzet)}</span></div>` : ''}
            ${bestDay && bestDay.omzet ? `<div class="bonus-row"><span class="bonus-row-label"><i class="fas fa-fire" style="color:var(--amber);margin-right:5px"></i>Hari Terbaik: ${formatDateLabel(bestDay.dateKey)}</span><span class="bonus-row-val" style="color:var(--amber)">Rp ${formatRupiah(bestDay.omzet)}</span></div>` : ''}
            ${topTx ? `<div class="bonus-row"><span class="bonus-row-label"><i class="fas fa-arrow-trend-up" style="color:var(--accent);margin-right:5px"></i>Trx Tertinggi: ${escapeHtml(topTx.name||topTx.user)}</span><span class="bonus-row-val" style="color:var(--accent)">Rp ${formatRupiah(topTx.amount)}</span></div>` : ''}
          </div>
        </div>

        <!-- Bonus Breakdown -->
        <div class="card page-in s3" style="padding:12px 13px">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:9px">
            <p class="label-xs" style="margin:0">Breakdown Bonus</p>
            <span class="user-chip">${escapeHtml(meta.key)}</span>
          </div>
          <div class="bonus-row"><span class="bonus-row-label"><i class="fas fa-percent" style="color:var(--accent);width:16px"></i> Bonus Trx Staff</span><span class="bonus-row-val" style="color:var(--accent)">Rp ${formatRupiah(totals.staffTrxBonus)}</span></div>
          <div class="bonus-row"><span class="bonus-row-label"><i class="fas fa-user-clock" style="color:var(--green);width:16px"></i> Gaji/Bonus Harian</span><span class="bonus-row-val" style="color:var(--green)">Rp ${formatRupiah(totals.dailySalary)}</span></div>
          <div class="bonus-row"><span class="bonus-row-label"><i class="fas fa-lock" style="color:var(--amber);width:16px"></i> Bonus Closing</span><span class="bonus-row-val" style="color:var(--amber)">Rp ${formatRupiah(totals.closingBonus)}</span></div>
          <div class="bonus-row"><span class="bonus-row-label"><i class="fas fa-pen-to-square" style="color:${totals.manualBonus<0?'var(--red)':'var(--amber)'};width:16px"></i> Bonus Manual</span><span class="bonus-row-val" style="color:${totals.manualBonus<0?'var(--red)':'var(--amber)'}">Rp ${formatRupiah(totals.manualBonus)}</span></div>
          <div class="bonus-row bonus-row-total"><span class="bonus-row-label">Total Dibayarkan</span><span class="bonus-row-val">Rp ${formatRupiah(totals.totalBonus)}</span></div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:8px">
            <div style="border:1px solid rgba(91,143,255,.18);border-radius:9px;background:var(--accent-dim);padding:8px;text-align:center">
              <div style="font-size:9px;font-weight:800;color:var(--accent);letter-spacing:.06em;text-transform:uppercase;margin-bottom:3px">Staff</div>
              <div style="font-family:var(--num-font);font-size:13px;font-weight:800;color:var(--accent)">Rp ${formatRupiah(totals.staffTotalBonus||0)}</div>
            </div>
            <div style="border:1px solid rgba(82,216,158,.18);border-radius:9px;background:var(--green-dim);padding:8px;text-align:center">
              <div style="font-size:9px;font-weight:800;color:var(--green);letter-spacing:.06em;text-transform:uppercase;margin-bottom:3px">Harian</div>
              <div style="font-family:var(--num-font);font-size:13px;font-weight:800;color:var(--green)">Rp ${formatRupiah(totals.dailyTotalSalary||0)}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- TAB: PER USER -->
      <div id="rep-users" class="report-section">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:9px">
          <p class="label-xs" style="margin:0">Rekap Per User</p>
          <span class="user-chip">${userRows.length} user</span>
        </div>
        ${userRows.length ? userRows.map((row, idx) => `
          <div class="report-user-row page-in">
            <div class="report-user-head">
              <div style="min-width:0;flex:1">
                <div class="report-user-name">#${idx+1} ${escapeHtml(row.name)}</div>
                <div class="report-user-meta">@${escapeHtml(row.user)} · ${getRoleDisplayName(row.role)} · ${row.transaksi} trx · ${row.hadir} hadir</div>
              </div>
              <div>
                <div class="report-user-omzet">Rp ${formatRupiah(row.omzet)}</div>
                <div class="report-user-omzet-lbl">omzet</div>
              </div>
            </div>
            ...
            <div class="report-user-total">
              <span style="font-size:10px;color:var(--text-ghost)">${row.hariTransaksi} hari · avg Rp ${formatRupiah(row.avgTransaction)}</span>
              <strong style="font-family:var(--num-font);font-size:12px;color:var(--amber)">Total Rp ${formatRupiah(row.totalBonus)}</strong>
            </div>
          </div>`).join('') :
          `<div style="font-size:12px;color:var(--text-soft);padding:18px;text-align:center;border:1px dashed var(--border);border-radius:12px">Belum ada data user bulan ini.</div>`
        }
      </div>

      <!-- TAB: HARIAN -->
      <div id="rep-daily" class="report-section">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:9px">
          <p class="label-xs" style="margin:0">Rekap Harian</p>
          <span class="user-chip">${meta.days} hari</span>
        </div>
        <!-- Mini search harian -->
        <div class="rh-search-wrap" style="margin-bottom:9px">
          <i class="fas fa-magnifying-glass"></i>
          <input type="text" class="g-input rh-search-input" placeholder="Cari tanggal, misal: 10 Jan..." oninput="window.__repDailySearch(this.value)" autocomplete="off">
        </div>
        <div id="rep-daily-list">
          ${dailyFiltered.length ? dailyFiltered.map(d => `
            <div class="report-daily-row">
              <div class="report-daily-date">${formatDateLabel(d.dateKey)}</div>
              <div>
                <div class="report-daily-omzet">Rp ${formatRupiah(d.omzet)}</div>
                <div class="report-daily-meta">${d.trxCount} trx · ${d.userCount} user · ${d.hadirCount} hadir</div>
              </div>
              <div class="report-daily-bonus">${d.closingBonus ? `+Rp ${formatRupiah(d.closingBonus)}` : ''}</div>
            </div>`).join('') :
            `<div style="font-size:12px;color:var(--text-soft);padding:18px;text-align:center;border:1px dashed var(--border);border-radius:12px">Belum ada aktivitas pada bulan ini.</div>`
          }
        </div>
      </div>

      <!-- TAB: DETAIL -->
      <div id="rep-detail" class="report-section">
        <div style="font-size:11px;font-weight:800;color:var(--amber);margin-bottom:8px"><i class="fas fa-pen-to-square"></i> Bonus Manual</div>
        ${manualBonuses.slice(0,12).length ? manualBonuses.slice(0,12).map(b => `
          <div style="display:flex;justify-content:space-between;gap:8px;padding:7px 0;border-bottom:1px solid var(--border);font-size:11px">
            <span>${escapeHtml(b.name||b.user)} · ${escapeHtml(b.note||b.reason||'Bonus manual')}</span>
            <b style="font-family:var(--num-font);color:${Number(b.amount||0)<0?'var(--red)':'var(--text)'}">Rp ${formatRupiah(b.amount)}</b>
          </div>`).join('') : `<div style="font-size:11px;color:var(--text-soft);margin-bottom:10px">Belum ada bonus manual bulan ini.</div>`}
        <div style="font-size:11px;font-weight:800;color:var(--accent);margin-top:12px;margin-bottom:8px"><i class="fas fa-arrow-trend-up"></i> Top 10 Transaksi</div>
        ${[...transactions].sort((a,b)=>Number(b.amount||0)-Number(a.amount||0)).slice(0,10).map(t => `
          <div style="display:flex;justify-content:space-between;gap:8px;padding:7px 0;border-bottom:1px solid var(--border);font-size:11px">
            <span style="min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${formatDateLabel(getTransactionDateKey(t))} · ${escapeHtml(t.name||t.user)} · ${escapeHtml(t.note||'Transaksi')}</span>
            <b style="font-family:var(--num-font);white-space:nowrap">Rp ${formatRupiah(t.amount)}</b>
          </div>`).join('') || `<div style="font-size:11px;color:var(--text-soft)">Belum ada transaksi bulan ini.</div>`}
      </div>
    `;
  }
  ...
  async function syncMonthlyReportData(monthKey = getReportMonthKey()) {
    ...
    try {
      ...
      console.error(e);
      showToast('Gagal sinkron data bulanan. Cek koneksi / index Firestore.', true);
    }
    setLoading(false);
  }
  ...
  function getMonthlyReportExportRows(data = buildMonthlyReportData(getReportMonthKey())) {
    ...
    rows.push({
      ...
      keterangan: topTransaction ? `${getTransactionDateKey(topTransaction) || '-'} · ${topTransaction.name || topTransaction.user || '-'} · ${getTransactionPaymentLabel(topTransaction)} · ${rupiahText(topTransaction.amount)}` : 'Belum ada data'
    });
  ...
  function exportMonthlyReportXlsx() {
    if (!window.XLSX) return showToast('Library Excel belum siap', true);
    const data = buildMonthlyReportData(getReportMonthKey());
    const selectedLabel = isAdmin() ? getSelectedUserLabel() : (currentUser?.name || 'data-saya');
    ...
    metric('Transaksi Tertinggi', topTransaction ? `${topTransaction.name || topTransaction.user || '-'} · ${getTransactionDateKey(topTransaction) || '-'}` : '-', topTransaction ? `${rupiah(topTransaction.amount)} · ${getTransactionPaymentLabel(topTransaction)} · ${topTransaction.note || ''}` : '');
  ...
  async function saveAdminUser() {
    ...
    try {
      ...
      await logAudit('user_upsert', { username, role, existed: !!existing, restored: !!wasInactive, transactionBonusPercent: role === 'admin' ? 0 : userBonusPercent, closingBonusPerMinute: (role === 'admin' || role === 'harian') ? 0 : userClosingPerMinute });
...
Masukkan PIN admin:`);
  ...
  function getBackupPayload() {
    return {
      version: 'firebase-admin-v9-backup-restore-update',
      projectId: firebaseConfig.projectId,
      exportedAt: new Date().toISOString(),
      exportedBy: currentUser?.username || 'admin',
      data: {
        users: sortUsers(cachedUsers).map(u => ({
          username: u.username, name: u.name, pin: String(u.pin || ''), role: String(u.role || 'staff'),
          transactionBonusRate: getUserTransactionBonusRate(u), transactionBonusPercent: getUserTransactionBonusPercent(u), closingBonusPerMinute: getUserClosingBonusPerMinute(u),
          dailyBonusRate: Number(u.dailyBonusRate || 0), dailyBonusPercent: Number(u.dailyBonusPercent || 0),
          deviceId: userDeviceLocked(u) ? String(u.deviceId || '') : '', deviceLocked: userDeviceLocked(u), deviceLabel: userDeviceLocked(u) ? String(u.deviceLabel || '') : '', deviceLockedAtMs: userDeviceLocked(u) ? Number(u.deviceLockedAtMs || 0) : 0, deviceLastLoginAtMs: Number(u.deviceLastLoginAtMs || 0), deviceResetAtMs: Number(u.deviceResetAtMs || 0),
          active: u.active !== false, createdAt: serializeTimestamp(u.createdAt) || null
        })),
        attendance: sortByCreatedDesc(getVisibleAttendance()).map(a => ({
          id: a.id || getAttendanceDocId(a.user, a.dateKey || extractDateKey(a)),
          user: a.user, name: a.name, dateKey: a.dateKey || extractDateKey(a),
          lat: a.lat ?? a.loc?.lat ?? null, lng: a.lng ?? a.loc?.lng ?? null,
          createdAt: serializeTimestamp(a.createdAt) || null,
          createdAtMs: a.createdAt?.toDate ? a.createdAt.toDate().getTime() : (a.createdAtMs || Date.now())
        })),
        transactions: sortByCreatedDesc(getVisibleTransactions()).map(t => ({
          id: t.id || '',
          user: t.user, name: t.name, note: t.note || 'Transaksi', amount: Number(t.amount || 0),
          paymentMethod: normalizeTransactionPaymentMethod(t.paymentMethod || t.paymentLabel || t.paymentType || t.metodePembayaran) || 'cash', paymentLabel: getTransactionPaymentLabel(t), paymentStatus: t.paymentStatus || 'success', paymentCashOutType: (normalizeTransactionPaymentMethod(t.paymentMethod || t.paymentLabel || t.paymentType || t.metodePembayaran) === 'qris_transfer' ? 'qris' : ''), isNonCashPayment: normalizeTransactionPaymentMethod(t.paymentMethod || t.paymentLabel || t.paymentType || t.metodePembayaran) === 'qris_transfer',
          dateKey: t.dateKey || extractDateKey(t), monthKey: t.monthKey || toMonthKey(new Date()),
          userRole: t.userRole || t.role || '', bonusGroup: t.bonusGroup || '', bonusRate: Number(t.bonusRate ?? t.transactionBonusRate ?? NaN), bonusPercent: Number(t.bonusPercent ?? t.transactionBonusPercent ?? NaN), bonusLogicVersion: Number(t.bonusLogicVersion || 1),
          createdAt: serializeTimestamp(t.createdAt) || null,
          createdAtMs: t.createdAt?.toDate ? t.createdAt.toDate().getTime() : (t.createdAtMs || Date.now())
        })),
        closings: sortByCreatedDesc((cachedClosings || []).filter(isClosingDataRecord)).map(c => serializeBackupRecord(c)),
        manualBonuses: sortByCreatedDesc((cachedManualBonuses || []).filter(b => b && b.deleted !== true && b.deleted !== 'true')).map(b => serializeBackupRecord(b)),
        bonusSettings: serializeBackupRecord({ ...(cachedBonusSettings || {}), transactionBonusRate: getTransactionBonusRate(), transactionBonusPercent: getTransactionBonusPercent(), closingBonusPerMinute: getClosingBonusPerMinute() }),
        headerGuideNote: serializeBackupRecord(cachedHeaderGuideNote || { id: HEADER_GUIDE_NOTE_DOC_ID, note: DEFAULT_HEADER_GUIDE_NOTE }),
        staffDailyHomeNote: serializeBackupRecord(cachedStaffDailyHomeNote || { id: STAFF_DAILY_NOTE_DOC_ID, note: DEFAULT_STAFF_DAILY_NOTE, enabled: true }),
        receiptTextSettings: serializeBackupRecord(getReceiptTextSettingsSafe()),
        rismaManualClosingConfig: serializeBackupRecord(cachedRismaManualClosingConfig || { id: RISMA_MANUAL_CLOSING_DOC_ID, enabled: true, allowedUsers: [], allowedNames: {} }),
        auditLogs: sortByCreatedDesc(cachedAuditLogs || []).map(a => serializeBackupRecord(a))
      }
    };
  }
  ...
  function saveEmergencyBackupBeforeRestore(label = 'restore') {
    try {
      const payload = getBackupPayload();
      payload.version = `${payload.version || 'firebase-admin-backup'}-emergency-before-${label}`;
  ...
  function isRestoreBackupShapeSafe(backup) {
    ...
    const counts = ['users','attendance','transactions','closings','manualBonuses','auditLogs']
      .reduce((sum, key) => sum + (Array.isArray(data[key]) ? data[key].length : 0), 0);
    const hasSettings = !!(data.bonusSettings || data.headerGuideNote || data.staffDailyHomeNote || data.receiptTextSettings || data.rismaManualClosingConfig);
  ...
  function saveBase64ToAndroidDownloads(filename, mimeType, base64Data) {
    ...
    try {
      bridge.saveFileToDownloads(String(filename || 'backup.txt'), String(mimeType || 'application/octet-stream'), String(base64Data || ''));
      return true;
    } catch (e) {
  ...
  async function restoreProFromFile() {
    ...
    try {
        ...
        backup = {
          ...
          data: {
            ...
            transactions: pickSheetRows(['transactions','transaction','transaksi','Transactions']).map(t => ({
              ...
              paymentCashOutType: normalizeTransactionPaymentMethod(t.paymentMethod || t.payment_method || t.paymentLabel || t.payment_label || t.metodePembayaran || t.metode_pembayaran) === 'qris_transfer' ? 'qris' : '',
              isNonCashPayment: normalizeTransactionPaymentMethod(t.paymentMethod || t.payment_method || t.paymentLabel || t.payment_label || t.metodePembayaran || t.metode_pembayaran) === 'qris_transfer',
      ...
      if (!confirm(getRestoreBackupSummaryText(backup, kind.toUpperCase()))) return;
      const emergencyFile = saveEmergencyBackupBeforeRestore(kind);
      ...
      await logAudit('restore_data', { format: kind, users: result.users.length, attendance: result.attendance.length, transactions: result.transactions.length, closings: result.closings.length, manualBonuses: result.manualBonuses.length });
      showToast(`✓ Restore ${kind.toUpperCase()} aman selesai: ${result.users.length} user, ${result.attendance.length} absen, ${result.transactions.length} transaksi, ${result.closings.length} closing · mode merge, data lama tidak dihapus`);
      await renderSettingsSummary();
      await refreshCurrentPage();
    } catch (e) {
      console.error(e);
  ...
  function resetRuntimeStateAfterDataWipe() {
    ...
    cachedReceiptTextSettings = { id: RECEIPT_TEXT_DOC_ID, ...DEFAULT_RECEIPT_TEXT_SETTINGS, updatedAtMs: 0, updatedBy: '', updatedByName: '' };
    cachedBonusSettings = { transactionBonusRate: DEFAULT_TRANSACTION_BONUS_RATE, closingBonusPerMinute: DEFAULT_CLOSING_BONUS_PER_MINUTE };
    cachedUsers = currentUser ? [{ ...currentUser, username: sanitizeUsername(currentUser.username || 'admin'), active: true }] : [];
    attendanceReady = false;
  ...
  async function resetAllAppData() {
    if (!isAdmin()) return showToast('Hanya admin!', true);
    if (isBusy) return showToast('Tunggu proses selesai', true);
    const adminUsername = sanitizeUsername(currentUser?.username || '');
    const pin = await askPin(`⚠️ Reset SEMUA data aplikasi?
Data transaksi, absen, closing, bonus manual, audit log, user non-admin, recycle bin, setting bonus, catatan header, catatan home staff, setting struk, dan config closing Risma akan dihapus/reset.

Akun admin yang sedang login tetap disimpan supaya aplikasi tidak terkunci.

Ketik PIN admin untuk lanjut:`);
    if (!pin) return;
    if (pin !== String(currentUser?.pin || '')) return showToast('PIN admin salah!', true);
    if (!confirm('FINAL: reset akan menghapus semua data operasional dan cache lokal. Lanjutkan?')) return;

    const emergencyFile = saveEmergencyBackupBeforeRestore('reset-data');
    if (!emergencyFile) {
    ...
    try {
      ...
      const deleted = {};
      deleted.transactions = await deleteCollectionCompletely('transactions');
      deleted.attendance = await deleteCollectionCompletely('attendance');
      deleted.closings = await deleteCollectionCompletely('closings');
      deleted.manualBonuses = await deleteCollectionCompletely('manualBonuses');
      deleted.audit_logs = await deleteCollectionCompletely('audit_logs');
      deleted.users = await deleteCollectionCompletely('users', {
        skip: (item) => item.id === adminUsername
      });

      resetRuntimeStateAfterDataWipe();
      bindRealtimeData();
      showToast(`✓ Reset selesai: ${Object.values(deleted).reduce((sum, n) => sum + n, 0)} data dihapus · backup darurat sudah dibuat`);
      await renderSettingsSummary();
      await refreshCurrentPage();
    } catch (e) {
      console.error(e);
  ...
  async function downloadBackup(format = 'json') {
    ...
    try {
      ...
      if (format === 'json') {
        ...
        const metaRows = [{ version: backup.version || '', projectId: backup.projectId || firebaseConfig.projectId, exportedAt: backup.exportedAt || '', exportedBy: backup.exportedBy || '', note: 'Backup lengkap: users, attendance, transactions, closings, manualBonuses, bonusSettings, headerGuideNote, staffDailyHomeNote, receiptTextSettings, rismaManualClosingConfig, auditLogs' }];
  ...
  async function importBackupData(backup) {
    ...
    const restoredAttendanceIds = new Set();
    const restoredTransactionIds = new Set();
    const restoredClosingIds = new Set();
    const restoredManualBonusIds = new Set();
    ...
    if (data.bonusSettings && typeof data.bonusSettings === 'object') {
      ...
      await setDoc(doc(db, 'closings', '__bonus_settings'), cleanFirestorePayload({
    ...
    if (data.rismaManualClosingConfig && typeof data.rismaManualClosingConfig === 'object') {
      ...
      await setDoc(doc(db, RISMA_MANUAL_CLOSING_COLLECTION, RISMA_MANUAL_CLOSING_DOC_ID), cleanFirestorePayload({
        id: RISMA_MANUAL_CLOSING_DOC_ID,
    ...
    if (headerGuideNote) {
      ...
      await setDoc(doc(db, 'closings', HEADER_GUIDE_NOTE_DOC_ID), cleanFirestorePayload({
        id: HEADER_GUIDE_NOTE_DOC_ID,
        type: 'header_guide_note',
    ...
    if (staffDailyHomeNote) {
      ...
      await setDoc(doc(db, 'closings', STAFF_DAILY_NOTE_DOC_ID), cleanFirestorePayload({
        id: STAFF_DAILY_NOTE_DOC_ID,
        type: 'staff_daily_home_note',
    ...
    if (receiptTextSettings) {
      ...
      await setDoc(doc(db, 'closings', RECEIPT_TEXT_DOC_ID), cleanFirestorePayload({
        ...receiptPayload,
        id: RECEIPT_TEXT_DOC_ID,
        dateKey: RECEIPT_TEXT_DOC_ID,
        closed: false,
    ...
    for (const u of users) {
      const username = sanitizeUsername(u.username);
      if (!username) continue;
      restoredUsernames.add(username);
      const role = normalizeRole(u.role || 'staff');
      ...
      await setDoc(doc(db, 'users', username), cleanFirestorePayload(userPayload), { merge: true });
    }

    for (const a of attendance) {
      const ms = Number(a.createdAtMs || (a.createdAt ? new Date(a.createdAt).getTime() : Date.now()));
      ...
      restoredAttendanceIds.add(attDocId);
      await setDoc(doc(db, 'attendance', attDocId), {
        ...
        deleted: false,
        restoredAt: serverTimestamp(),
        restoredBy: currentUser?.username || 'admin'
      }, { merge: true });
    }
    ...
    for (const t of transactions) {
      ...
      const ms = Number(sourceCreatedAtMs || sourceCreatedAtParsed || Date.now());
      ...
      const existingTxId = await findExistingTransactionForRestore(restorePayloadForCheck);
      const txDocId = existingTxId || getStableTransactionRestoreId(restorePayloadForCheck);
      restoredTransactionIds.add(txDocId);
      await setDoc(doc(db, 'transactions', txDocId), payload, { merge: true });
    }
    ...
    for (const c of closings) {
      ...
      await setDoc(doc(db, 'closings', id), payload, { merge: true });
    }

    for (const b of manualBonuses) {
      const user = sanitizeUsername(b.user || '');
      if (!user) continue;
      const dateKey = String(b.dateKey || '').slice(0, 10) || toDateKey(new Date(Number(b.createdAtMs || Date.now())));
      const monthKey = String(b.monthKey || dateKey.slice(0, 7)).slice(0, 7);
      const id = safeRestoreDocId(b.id || `manual_${user}_${dateKey}_${Number(b.amount || 0)}_${Number(b.createdAtMs || Date.now())}_${stableRestoreHash(String(b.note || b.reason || 'Bonus manual'))}`);
      const bonusRole = restoredRoleForUser(user, b.userRole || b.role || b.bonusGroup || '');
      restoredManualBonusIds.add(id);
      await setDoc(doc(db, 'manualBonuses', id), cleanFirestorePayload({
        ...b,
        id,
        user,
    ...
    for (const a of auditLogs.slice(0, AUDIT_SYNC_LIMIT)) {
      const id = String(a.id || `restore_log_${Date.now()}_${Math.random().toString(36).slice(2)}`).replace(/[^a-zA-Z0-9_\-.]/g, '_').slice(0, 160);
      await setDoc(doc(db, 'audit_logs', id), cleanFirestorePayload({ ...a, id, restoredAt: serverTimestamp(), restoredBy: currentUser?.username || 'admin' }), { merge: true });
    }

    // MODE AMAN: jangan hapus / soft-delete data aktif yang tidak ada di file backup.
    // Restore lama pernah dibuat "exact backup" dan itu berisiko membuat data terlihat hilang
    // kalau backup parsial atau format lama tidak membawa semua koleksi. Sekarang restore hanya MERGE.
    ...
    return { users, attendance, transactions, closings, manualBonuses, auditLogs, headerGuideNote, staffDailyHomeNote, receiptTextSettings, cleanupResult };
  }
  ...
  async function restoreBackupFromFile() {
    ...
    try {
      ...
      if (!isRestoreBackupShapeSafe(backup)) throw new Error('Isi backup kosong / tidak terbaca');
      if (!confirm(getRestoreBackupSummaryText(backup, 'JSON'))) return;
      const emergencyFile = saveEmergencyBackupBeforeRestore('json');
      if (emergencyFile) console.log('Emergency backup dibuat:', emergencyFile);
      const result = await importBackupData(backup);
      if ($('restore-file')) $('restore-file').value = '';
      await logAudit('restore_data', { format: 'json', users: result.users.length, attendance: result.attendance.length, transactions: result.transactions.length, closings: result.closings.length, manualBonuses: result.manualBonuses.length });
      showToast(`✓ Restore aman selesai: ${result.users.length} user, ${result.attendance.length} absen, ${result.transactions.length} transaksi, ${result.closings.length} closing · tidak ada data lama yang dihapus`);
      await renderSettingsSummary();
      await refreshCurrentPage();
    } catch (e) {
      console.error(e);
  ...
  async function restoreExcelFromFile() {
    ...
    try {
      ...
      const backup = {
        ...
        data: {
          users: pickSheetRows(['users','user','Users','User']).map(u => ({
            ...
            dailyBonusPercent: Number(u.dailyBonusPercent || u.daily_percent || u.persen_harian || 0),
            dailyBonusRate: Number(u.dailyBonusRate || 0) || (Number(u.dailyBonusPercent || u.daily_percent || u.persen_harian || 0) / 100),
            active: String(u.active || 'true') !== 'false',
      ...
      if (!confirm(getRestoreBackupSummaryText(backup, 'EXCEL'))) return;
      const emergencyFile = saveEmergencyBackupBeforeRestore('excel');
      if (emergencyFile) console.log('Emergency backup dibuat:', emergencyFile);
      const result = await importBackupData(backup);
      if ($('restore-excel-file')) $('restore-excel-file').value = '';
      await logAudit('restore_data', { format: 'excel', users: result.users.length, attendance: result.attendance.length, transactions: result.transactions.length, closings: result.closings.length, manualBonuses: result.manualBonuses.length });
      showToast(`✓ Restore Excel aman selesai: ${result.users.length} user, ${result.attendance.length} absen, ${result.transactions.length} transaksi, ${result.closings.length} closing · tidak ada data lama yang dihapus`);
      await renderSettingsSummary();
      await refreshCurrentPage();
    } catch (e) {
      console.error(e);
  ...
  const ADMIN_PANEL_KEYS = ['attendance','manual_bonus','receipt_text','header_guide_note','daily_employee','user_management','backup_restore','deleted_data'];
  ...
  function renderAdminSidebarPanelCard(title, badgeHtml = '', bodyHtml = '', icon = 'fa-layer-group', subtitle = '') {
    return `
      <div class="card" style="padding:0;margin-top:12px;overflow:hidden;border-color:rgba(0,168,120,.22);box-shadow:var(--kp-shadow)">
        <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;padding:14px 16px;background:linear-gradient(180deg,#effbf7,#ffffff);border-bottom:1px solid var(--border)">
          <div style="display:flex;align-items:center;gap:12px;min-width:0;flex:1">
            <div style="width:40px;height:40px;border-radius:12px;background:var(--kp-green)!important;color:white;display:flex;align-items:center;justify-content:center;flex-shrink:0"><i class="fas ${icon}" style="font-size:14px"></i></div>
            <div style="min-width:0;flex:1">
              <div style="font-size:14px;font-weight:900;letter-spacing:-.01em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${escapeHtml(title)}</div>
              <div style="font-size:11px;color:var(--text-soft);margin-top:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${escapeHtml(subtitle || 'Panel dari sidebar kiri')}</div>
            </div>
          </div>
          <div style="display:flex;align-items:center;gap:8px;flex-shrink:0">${badgeHtml}</div>
        </div>
        <div style="padding:14px 16px 16px;background:#fff">${bodyHtml}</div>
      </div>`;
  }
...
Masukkan PIN admin:`);
  ...
  function renderDailyEmployeeControlBody() {
    const bonusUsers = activeUsers().filter(u => ['staff','harian'].includes(normalizeRole(u.role || 'staff')));
    return `
      <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;margin-bottom:10px">
    ...
    return `
      ...
      <div style="display:grid;grid-template-columns:1fr;gap:9px">
        <select id="admin-user-select" class="g-input" onchange="fillAdminUserForm(this.value)">
          ...
          ${sortUsers(cachedUsers).map(u => `<option value="${escapeHtml(u.username)}">${escapeHtml(u.name)} (@${escapeHtml(u.username)}) · ${escapeHtml(getRoleDisplayName(u.role || 'staff').toUpperCase())} ${isUserActive(u) ? '' : '· NONAKTIF'}</option>`).join('')}
        </select>
    ...
    return `
      ...
      <div style="display:flex;flex-direction:column;gap:9px">
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px">
          ...
          <button onclick="downloadBackup('xlsx')" class="btn btn-glass" style="padding:12px;font-size:12px"><i class="fas fa-file-excel"></i> Backup XLSX</button>
        ...
        <div style="border:1px dashed var(--border);border-radius:14px;padding:12px;background:var(--surface)">
          <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px">
            ...
            <div style="min-width:0;flex:1"><div style="font-size:13px;font-weight:800">Restore Otomatis</div><div style="font-size:11px;color:var(--text-soft);margin-top:2px">Satu input baca JSON / XLS / XLSX, termasuk setting baru dan menu sidebar admin.</div></div>
          </div>
      ...
      <p style="font-size:11px;color:var(--text-soft);margin-top:10px">Alur aman: backup dulu → reset bila perlu → restore → cek hasil → normalisasi data lama. Backup JSON/XLSX ikut menyimpan user, transaksi, absen, closing, bonus manual, catatan header, catatan home staff, setting struk, config closing Risma, dan audit log.</p>`;
  }
  ...
  async function renderSettingsSummary() {
    ...
    const adminAttendanceRows = isAdmin() ? activeUsers().map(u => ({ user: u, perf: getMonthlyAttendancePerformance(u.username) })) : [];

    const inactiveUsersHtml = `
      <div style="margin-top:14px;padding-top:13px;border-top:1px solid var(--border)">
        <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;margin-bottom:10px">
          <p class="label-xs" style="margin:0">User Nonaktif</p>
          <span class="status-pill status-warn">${inactiveUsers.length} user</span>
        </div>
        <div style="display:flex;flex-direction:column;gap:8px;max-height:220px;overflow-y:auto">
          ${visibleInactiveUsers.length ? visibleInactiveUsers.map(u => `<div style="display:flex;align-items:center;justify-content:space-between;gap:10px;padding:10px 12px;border:1px solid var(--border);border-radius:12px;background:var(--surface)"><div style="min-width:0;flex:1"><div style="font-size:12px;font-weight:700">${escapeHtml(u.name || u.username)} (@${escapeHtml(u.username || '-')})</div><div style="font-size:10px;color:var(--text-soft)">${escapeHtml(getRoleDisplayName(u.role || 'staff').toUpperCase())} · nonaktif</div></div><button onclick="restoreAdminUser('${escapeHtml(u.username || '')}')" class="btn btn-success" style="padding:8px 10px;font-size:10.5px;border-radius:11px;white-space:nowrap"><i class="fas fa-rotate-left"></i> Aktifkan</button></div>`).join('') : `<div style="font-size:12px;color:var(--text-soft)">Belum ada user nonaktif.</div>`}
        </div>
        ${hasMoreInactiveUsers ? `<button onclick="increaseInactiveUserLimit()" class="btn btn-glass" style="width:100%;margin-top:10px;padding:11px;font-size:12px">Muat ${Math.min(INACTIVE_USER_STEP, inactiveUsers.length - visibleInactiveUsers.length)} user nonaktif lagi</button>` : ''}
      </div>`;

    const activeAdminPanelKey = ADMIN_PANEL_KEYS.find(k => isAdminSettingPanelOpen(k));
    const adminPanelConfig = activeAdminPanelKey ? {
      attendance: {
        title: 'Performa Absen Staff',
        badgeHtml: `<span class="status-pill status-ok">${getStaffBonusUsers().length}</span>`,
        icon: 'fa-user-clock',
        subtitle: 'Rata-rata jam absen bulanan staff',
        body: () => renderAdminAttendancePerformanceBody()
      },
      manual_bonus: {
        title: 'Tambah Bonus Manual',
        badgeHtml: `<span class="status-pill status-ok">Rp ${formatRupiah(getMonthlyManualBonus(null, toMonthKey(nowDate())))}</span>`,
        icon: 'fa-hand-holding-dollar',
        subtitle: 'Tambah bonus ke staff/harian pilihan',
        body: () => renderManualBonusCard()
      },
      receipt_text: {
        title: 'Setting Tulisan Struk',
        badgeHtml: '<span class="status-pill status-ok">Admin</span>',
        icon: 'fa-receipt',
        subtitle: 'Ubah nama toko, label, footer, dan jarak sobek',
        body: () => renderReceiptTextSettingsBody()
      },
      header_guide_note: {
        title: 'Catatan Panduan Header',
        badgeHtml: '<span class="status-pill status-ok">Edit</span>',
        icon: 'fa-pen-to-square',
        subtitle: 'Ubah catatan saat Panduan Icon Header di-hide',
        body: () => renderHeaderGuideNoteControlBody()
      },
      daily_employee: {
        title: 'Kontrol Bonus Khusus User',
        badgeHtml: `<span class="status-pill status-ok">${activeUsers().filter(u => ['staff','harian'].includes(normalizeRole(u.role || 'staff'))).length}</span>`,
        icon: 'fa-user-tag',
        subtitle: 'Atur bonus transaksi per user langsung',
        body: () => renderDailyEmployeeControlBody()
      },
      user_management: {
        title: 'Manajemen User',
        badgeHtml: `<span class="status-pill status-ok">${userCount}</span>`,
        icon: 'fa-users-gear',
        subtitle: 'Tambah, edit, nonaktifkan, dan aktifkan user',
        body: () => renderUserManagementBody(userCount) + inactiveUsersHtml
      },
      backup_restore: {
        title: 'Backup, Reset & Restore',
        badgeHtml: '<span class="status-pill status-warn">Backup</span>',
        icon: 'fa-database',
        subtitle: 'Backup, restore otomatis, normalisasi',
        body: () => renderBackupRestoreBody()
      },
      deleted_data: {
        title: 'Data Terhapus',
        badgeHtml: `<span class="status-pill status-warn">${deletedTransactions.length}</span>`,
        icon: 'fa-trash-can-arrow-up',
        subtitle: 'Recycle bin transaksi',
        body: () => renderDeletedDataBody(filteredDeletedTransactions, visibleDeletedTransactions, hasMoreDeleted, deletedTransactions)
      }
    }[activeAdminPanelKey] : null;

    const adminBlocks = (isAdmin() && adminMoreOpen && adminPanelConfig)
      ? renderAdminSidebarPanelCard(adminPanelConfig.title, adminPanelConfig.badgeHtml, adminPanelConfig.body(), adminPanelConfig.icon, adminPanelConfig.subtitle)
      : '';

    const firebaseInfoBlock = `
      <div style="display:flex;flex-direction:column;gap:8px">
        <div style="display:flex;justify-content:space-between;font-size:12px"><span>Project</span><strong>${escapeHtml(firebaseConfig.projectId)}</strong></div>
        <div style="display:flex;justify-content:space-between;font-size:12px"><span>Status Koneksi</span><strong>${navigator.onLine ? 'Online' : 'Offline'}</strong></div>
        <div style="display:flex;justify-content:space-between;font-size:12px"><span>Role Aktif</span><strong>${getRoleLabel()}</strong></div>
        <div style="display:flex;justify-content:space-between;font-size:12px"><span>Scope Data</span><strong>${escapeHtml(getUserDataScopeLabel())}</strong></div>
        <div style="display:flex;justify-content:space-between;font-size:12px"><span>Transaksi Aktif</span><strong>${getVisibleTransactions().length}</strong></div>
        <div style="display:flex;justify-content:space-between;font-size:12px"><span>Recycle Bin</span><strong>${deletedTransactions.length}</strong></div>
      </div>`;
    info.innerHTML = (isAdmin() && adminMoreOpen && adminPanelConfig) ? adminBlocks : `${firebaseInfoBlock}${adminBlocks}`;
  ...
  function normalizeStaffDailyNoteUrl(rawUrl) {
  ...
  function openStaffDailyNoteLink(event, rawUrl) {
    if (event) event.preventDefault();
    const url = normalizeStaffDailyNoteUrl(rawUrl);
    if (!/^https?:\/\//i.test(url)) return false;
    try {
      const bridge = window.Android;
      const methods = ['openExternalUrl', 'openExternalLink', 'openUrl', 'openBrowser'];
      if (bridge) {
      ...
      console.warn('open external link bridge failed', e);
    }
  ...
  function renderStaffDailyNoteLinks(value) {
    ...
    text.replace(urlRegex, (match, url, offset) => {
      ...
      let label = url;
      let trailing = '';
      const trailingMatch = label.match(/[),.;!?]+$/);
      if (trailingMatch) {
        trailing = trailingMatch[0];
        label = label.slice(0, -trailing.length);
      }
      const href = normalizeStaffDailyNoteUrl(label);
      if (/^https?:\/\//i.test(href)) {
        html += `<a class="staff-daily-link" href="${escapeHtml(href)}" target="_blank" rel="noopener noreferrer" onclick="return openStaffDailyNoteLink(event, this.href)">${escapeHtml(label)}</a>${escapeHtml(trailing)}`;
      } else {
      ...
      <div class="trx-card" style="align-items:flex-start">
        ...
        <div style="min-width:0;flex:1">
          <div style="display:flex;align-items:center;justify-content:space-between;gap:8px">
            ...
            <span class="status-pill ${deletedCls}" style="font-size:9px;padding:3px 7px;white-space:nowrap">${deletedText}</span>
          </div>
          <div style="display:flex;gap:7px;margin-top:9px;flex-wrap:wrap">
            ${isRecordDeleted(a) ? `
              <button onclick="restoreAdminAttendance('${escapeHtml(a.id)}')" class="btn btn-glass" style="padding:8px 10px;font-size:11px"><i class="fas fa-rotate-left"></i> Pulihkan</button>
            ` : `
              <button onclick="openAdminAttendanceModal('${escapeHtml(a.id)}')" class="btn btn-glass" style="padding:8px 10px;font-size:11px"><i class="fas fa-pen"></i> Edit</button>
              <button onclick="deleteAdminAttendance('${escapeHtml(a.id)}')" class="btn btn-danger" style="padding:8px 10px;font-size:11px"><i class="fas fa-trash"></i> Hapus</button>
            `}
          </div>
        </div>
      </div>`;
  ...
  function openAdminAttendanceModal(id = '') {
    ...
    wrap.style.justifyContent = 'center';
    wrap.innerHTML = `
      <div class="modal-sheet" style="max-width:392px;border-radius:22px;padding:16px">
        <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:12px">
          <div style="min-width:0">
    ...
    document.body.appendChild(wrap);
    wrap.addEventListener('click', (e) => { if (e.target === wrap) closeAdminAttendanceModal(); });
  }
  ...
  function applyNavRoleLabels() {
    ...
    const labels = adminMode
      ? { home:'Admin', history:'Semua', attendance:'Absen', closing:'Closing', report:'Global', settings:'Sistem' }
      : { home:'Beranda', history:'Hari Ini', attendance:'Absen', closing:'Closing', report:'Laporan', settings:'Setelan' };
    ...
    if (attendanceBtn) attendanceBtn.style.display = adminMode ? 'flex' : 'none';
    if (closingBtn) closingBtn.style.display = adminMode ? 'flex' : 'none';
    if (reportBtn) reportBtn.style.display = adminMode ? 'flex' : 'none';
    if (settingsBtn) settingsBtn.style.display = adminMode ? 'none' : 'flex';
    document.querySelectorAll('.admin-side-extra').forEach(btn => btn.style.display = adminMode ? 'flex' : 'none');
    document.querySelectorAll('.admin-sidebar-sep').forEach(sep => sep.style.display = adminMode ? 'block' : 'none');
  }


  const ADMIN_SIDEBAR_PANEL_NAV = {
    attendance: 'nav-admin-attendance-performance',
    manual_bonus: 'nav-admin-manual-bonus',
    receipt_text: 'nav-admin-receipt-text',
    header_guide_note: 'nav-admin-header-note',
    daily_employee: 'nav-admin-daily-employee',
    user_management: 'nav-admin-user-management',
    backup_restore: 'nav-admin-backup-restore',
    deleted_data: 'nav-admin-deleted-data'
  };

  async function openNormalSettings() {
    if (isAdmin()) {
      adminMoreOpen = false;
      ADMIN_PANEL_KEYS.forEach(k => adminSettingPanels[k] = false);
      saveAdminPanelState();
  ...
  async function openAdminSidebarPanel(key) {
    if (!isAdmin()) return;
    if (!ADMIN_PANEL_KEYS.includes(key)) return;
    adminMoreOpen = true;
    ADMIN_PANEL_KEYS.forEach(k => adminSettingPanels[k] = false);
    adminSettingPanels[key] = true;
    saveAdminPanelState();
    await router.navigate('settings');
    setTimeout(() => {
      const el = document.getElementById('settings-summary');
      if (el && window.matchMedia('(min-width: 760px)').matches) el.scrollIntoView({ behavior:'smooth', block:'start' });
    }, 60);
  }

  async function openAdminSidebarLogs() {
    if (!isAdmin()) return;
    await router.navigate('adminLogs');
  }
  ...
  function setActiveNav(page) {
    ...
    if (navEl) {
      navEl.classList.toggle('staff-nav', !!currentUser && !isAdmin());
      navEl.classList.toggle('admin-six-nav', !!currentUser && isAdmin());
    }
    const appEl = $('app');
    if (appEl) appEl.classList.toggle('admin-sidebar-mode', !!currentUser && isAdmin());
    ...
    Object.entries(ADMIN_SIDEBAR_PANEL_NAV).forEach(([key, id]) => {
      const btn = $(id);
      if (btn) btn.classList.toggle('active', page === 'settings' && adminMoreOpen && isAdminSettingPanelOpen(key));
    });
    const logBtn = $('nav-admin-logs');
    if (logBtn) logBtn.classList.toggle('active', page === 'adminLogs');
    const fab = $('fab-pos');
    if (!fab) return;
    fab.classList.toggle('staff-fab', !!currentUser && !isAdmin());
    const adminSidebarPanelActive = isAdmin() && page === 'settings' && adminMoreOpen && ADMIN_PANEL_KEYS.some(k => isAdminSettingPanelOpen(k));
    fab.style.display = currentUser && (isAdmin() ? (['home','history','report'].includes(page) || (page === 'settings' && !adminSidebarPanelActive)) : page === 'home') ? 'flex' : 'none';
  }


  function handleStaffSwipeStart(e) {
    if (!currentUser || isAdmin()) return;
    const touch = e.touches && e.touches[0];
    if (!touch) return;
    staffSwipeStartX = touch.clientX;
    staffSwipeStartY = touch.clientY;
    staffSwipeStartT = Date.now();
  }

  function handleStaffSwipeEnd(e) {
    if (!currentUser || isAdmin()) return;
    if (!['home','history'].includes(currentPage)) return;
    const touch = e.changedTouches && e.changedTouches[0];
    if (!touch || !staffSwipeStartT) return;
    const dx = touch.clientX - staffSwipeStartX;
    const dy = touch.clientY - staffSwipeStartY;
    const dt = Date.now() - staffSwipeStartT;
    staffSwipeStartT = 0;
    if (Math.abs(dx) < 70 || Math.abs(dx) < Math.abs(dy) * 1.35 || dt > 900) return;
    if (dx < 0 && currentPage === 'home') router.navigate('history');
    if (dx > 0 && currentPage === 'history') router.navigate('home');
  }

  const router = {
    navigate: async (page) => {
      if (window.closeMobileSidebar) window.closeMobileSidebar(true);
      currentPage = page;
      ...
      const themeSwitchIcon = `<button class="theme-toggle theme-btn" onclick="toggleTheme()" title="Mode terang/gelap" aria-label="Mode terang/gelap">${themeIcon}</button>`;
      const modeIconClass = getAppViewMode() === 'android' ? 'fa-mobile-screen-button' : 'fa-desktop';
      const modeTitle = getAppViewMode() === 'android' ? 'Mode Android aktif' : 'Mode Desktop aktif';
      const viewModeIcon = `<button class="theme-toggle view-mode-btn" onclick="toggleAppViewMode()" title="${modeTitle}" aria-label="${modeTitle}"><i class="fas ${modeIconClass}"></i></button>`;
      const logoutIcon = '<button class="theme-toggle logout-btn" onclick="logout()" title="Keluar Aplikasi" aria-label="Keluar Aplikasi"><i class="fas fa-right-from-bracket"></i></button>';
      const mobileMenuIcon = '<button class="theme-toggle mobile-menu-btn" onclick="openMobileSidebar()" title="Buka menu" aria-label="Buka menu"><i class="fas fa-bars"></i></button>';
      const systemIcon = `${mobileMenuIcon}${viewModeIcon}${themeSwitchIcon}${logoutIcon}`; // Tombol cepat: menu mobile, mode desktop/android, tema, logout.
      if (!window.__staffSwipeReady && content) {
        content.addEventListener('touchstart', handleStaffSwipeStart, { passive:true });
        content.addEventListener('touchend', handleStaffSwipeEnd, { passive:true });
        window.__staffSwipeReady = true;
      }

      if (page === 'login') {
        nav.style.display = 'none';
        $('fab-pos').style.display = 'none';
        content.innerHTML = `
          <div style="min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:32px 22px;position:relative">
            ...
            <div class="login-brand page-in" style="margin-bottom:18px"><div style="font-size:34px">🧕</div></div>
            ...
            <p style="font-size:13px;color:var(--text-soft);margin-bottom:10px;text-align:center" class="page-in s1">Panel Kasir & Admin</p>
      ...
      if (page === 'home') {
        ...
        content.innerHTML = isAdmin() ? `
          <div class="page-pad" style="padding-top:8px">
            <div style="padding:10px 0 8px;display:flex;justify-content:space-between;align-items:center" class="page-in">
              <div>
                ...
                <h2 style="font-size:18px;font-weight:850;letter-spacing:-.03em">Dashboard</h2>
              </div>
            ...
            <div class="card page-in s1" style="padding:10px 11px;margin-bottom:8px">
              <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap">
                <div id="home-status"></div>
                <div id="loc-info"><span class="loc-chip loc-loading" style="padding:4px 8px;font-size:10px"><i class="fas fa-circle-notch fa-spin" style="font-size:9px"></i> GPS...</span></div>
              </div>
            </div>
          ...
          <div class="page-pad" style="padding-bottom:98px">
            <div style="padding:18px 0 12px;display:flex;justify-content:space-between;align-items:center;gap:12px" class="page-in">
              <div style="min-width:0">
                <p style="font-size:11px;color:var(--text-ghost);font-weight:700;margin-bottom:3px">Halo, ${escapeHtml(currentUser.name)}</p>
                <h2 style="font-size:20px;font-weight:850;letter-spacing:-.03em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">Bismillah Dulu</h2>
              </div>
              <div class="header-actions">
                ${memberIcon}
                ${systemIcon}
            ...
            ${renderStaffDailyHomeNotePublicCard()}
            ${isDailyEmployee(currentUser) ? `
            <div class="stat-hero page-in s1" style="margin-bottom:10px">
              <p class="stat-label">Pendapatan Hari Ini</p>
              <p id="today-income" class="stat-value">Rp 0</p>
              <p class="stat-sub">Data transaksi hari ini saja</p>
            </div>
            <div class="bonus-card page-in s2" style="padding:14px 15px;margin-bottom:9px">
              <div style="display:flex;align-items:center;justify-content:space-between;gap:10px">
                <div>
                  <p class="label-xs" style="margin-bottom:6px;color:var(--amber)">Gaji Hari Ini</p>
                  <p id="total-bonus" style="font-family:var(--num-font);font-size:23px;font-weight:850;color:var(--amber);line-height:1">Rp 0</p>
                  <p id="bonus-text" style="font-size:10.5px;color:var(--text-soft);margin-top:5px"></p>
                </div>
                <span id="scope-chip" class="user-chip">Harian</span>
              </div>
            </div>` : `
            <div class="card page-in s1" style="padding:13px 14px;margin-bottom:10px">
              <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:10px">
                <p class="label-xs" style="margin:0">Absen Hari Ini</p>
                <div id="loc-info"><span class="loc-chip loc-loading"><i class="fas fa-circle-notch fa-spin" style="font-size:10px"></i> GPS...</span></div>
              </div>
              <div style="display:flex;align-items:center;justify-content:space-between;gap:10px">
                <div id="home-status"></div>
                <div id="today-attendance-card" style="min-width:82px;text-align:center;border:1px solid rgba(255,107,107,0.2);border-radius:13px;padding:9px 10px;background:var(--red-dim)">
                  <p id="today-attendance-label" style="font-size:9px;font-weight:800;letter-spacing:.06em;text-transform:uppercase;color:var(--red);margin-bottom:4px">Jam</p>
                  <p id="today-attendance-info" style="font-family:var(--num-font);font-size:18px;font-weight:800;color:var(--red);line-height:1">--:--</p>
                </div>
              </div>
            </div>
            <div id="attendance-performance-card" class="card page-in s2" style="padding:13px 14px;margin-bottom:10px"></div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:9px;margin-bottom:9px" class="page-in s2">
              <div class="card" style="padding:13px 13px"><p class="label-xs" style="margin-bottom:8px">Pendapatan Hari Ini</p><p id="today-income" style="font-family:var(--num-font);font-size:17px;font-weight:800;color:var(--text)">Rp 0</p></div>
              <div class="card" style="padding:13px 13px"><p class="label-xs" style="margin-bottom:8px">Pendapatan Bulanan</p><p id="month-omzet" style="font-family:var(--num-font);font-size:17px;font-weight:800;color:var(--green)">Rp 0</p></div>
            </div>
            <div class="bonus-card page-in s3" style="padding:14px 15px;margin-bottom:9px">
              <div style="display:flex;align-items:center;justify-content:space-between;gap:10px"><div><p class="label-xs" style="margin-bottom:6px;color:var(--amber)">Bonus ++ </p><p id="total-bonus" style="font-family:var(--num-font);font-size:23px;font-weight:850;color:var(--amber);line-height:1">Rp 0</p><p id="bonus-text" style="font-size:10.5px;color:var(--text-soft);margin-top:5px">Bonus ini sudah termasuk total bonus keseluruhan</p></div><span id="scope-chip" class="user-chip">Data Saya</span></div>
              <div id="staff-closing-status" style="margin-top:10px"></div>
            </div>
            `}
            ${renderHeaderIconGuide()}
            <button onclick="router.navigate('history')" class="btn btn-glass page-in s4" style="width:100%;padding:13px 14px;font-size:13px;border-radius:14px;margin-top:2px;justify-content:space-between">
              <span><i class="fas fa-receipt"></i> Lihat Riwayat Hari Ini</span>
              <i class="fas fa-chevron-right"></i>
            </button>
            <div class="swipe-hint page-in s4"><i class="fas fa-hand-pointer"></i> Geser kiri untuk Riwayat Hari Ini</div>
          </div>`;
        ...
        if (!isAdmin()) {
          content.innerHTML = `
            <div class="page-pad">
              <div style="padding:18px 0 14px;display:flex;justify-content:space-between;align-items:center;gap:12px" class="page-in">
                <div style="display:flex;align-items:center;gap:10px;min-width:0">
                  <button onclick="router.navigate('home')" class="theme-toggle" style="width:38px;height:38px"><i class="fas fa-arrow-left"></i></button>
                  <div style="min-width:0">
                    <p class="page-title" style="font-size:20px">Riwayat Hari Ini</p>
                    <p class="page-subtitle">Khusus transaksi akun kamu hari ini</p>
                  </div>
                </div>
                <div class="header-actions">
                  ${memberIcon}
              ...
              <div class="swipe-hint page-in s1" style="margin-bottom:10px"><i class="fas fa-arrow-left"></i> Geser kanan untuk Beranda</div>
              <div id="staff-today-history-page"></div>
            </div>`;
          renderStaffTodayHistoryPage();
          return;
        }
        content.innerHTML = `
          <div class="page-pad">
            <div style="padding:34px 0 18px;display:flex;justify-content:space-between;align-items:center" class="page-in">
              <div><p class="page-title">Semua Transaksi</p><p class="page-subtitle">Pantau transaksi seluruh karyawan</p></div>
        ...
        content.innerHTML = `
          <div class="page-pad">
            <div style="padding:34px 0 18px;display:flex;justify-content:space-between;align-items:center;gap:10px" class="page-in">
              <div style="min-width:0">
                <p class="page-title" style="font-size:21px">Absensi Staff</p>
                <p class="page-subtitle">Kelola data absen seluruh karyawan</p>
              </div>
              <div class="header-actions">
                ${memberIcon}
                ${systemIcon}
        ...
        content.innerHTML = `
          <div class="page-pad">
            <div style="padding:34px 0 18px;display:flex;justify-content:space-between;align-items:center" class="page-in">
              <div><p class="page-title">${isAdmin() ? 'Laporan Global' : 'Laporan'}</p><p class="page-subtitle">${isAdmin() ? 'Ringkasan semua user dalam satu panel' : 'Omzet, hadir, bonus, dan ringkasan kerja'}</p></div>
              <div style="display:flex;gap:8px">
                ${memberIcon}
                ${systemIcon}
              </div>
        ...
        content.innerHTML = `
          <div class="page-pad">
            <div style="padding:50px 0 18px;display:flex;justify-content:space-between;align-items:center;gap:10px" class="page-in">
              <div style="display:flex;align-items:center;gap:10px;min-width:0">
                <button onclick="openNormalSettings()" class="theme-toggle" style="width:38px;height:38px"><i class="fas fa-arrow-left"></i></button>
                <div style="min-width:0">
                  <p class="page-title" style="font-size:21px">Log Aktivitas Admin</p>
                  <p class="page-subtitle">Halaman khusus audit log</p>
                </div>
              </div>
              <div class="header-actions">${memberIcon}${systemIcon}</div>
            </div>
        ...
        if (!isAdmin()) return router.navigate('home');
        nav.style.display = 'flex'; setActiveNav(page);
        const activePanelKey = adminMoreOpen ? ADMIN_PANEL_KEYS.find(k => isAdminSettingPanelOpen(k)) : '';
        const activePanelMeta = {
          attendance: { title:'Performa Absen', subtitle:'Rata-rata jam absen bulanan staff' },
          manual_bonus: { title:'Bonus Manual', subtitle:'Tambah bonus ke staff/harian pilihan' },
          receipt_text: { title:'Tulisan Struk', subtitle:'Ubah tulisan struk dan footer' },
          header_guide_note: { title:'Catatan Header', subtitle:'Ubah catatan panduan header' },
          daily_employee: { title:'Bonus User', subtitle:'Atur bonus transaksi per user' },
          user_management: { title:'Manajemen User', subtitle:'Tambah, edit, dan nonaktifkan user' },
          backup_restore: { title:'Backup Restore', subtitle:'Backup, restore, dan normalisasi data' },
          deleted_data: { title:'Data Terhapus', subtitle:'Pulihkan atau hapus permanen transaksi' }
        }[activePanelKey] || null;

        if (activePanelMeta) {
          content.innerHTML = `
          <div class="page-pad admin-panel-only-page">
            <div style="padding:50px 0 18px;display:flex;justify-content:space-between;align-items:center;gap:10px" class="page-in">
              <div style="min-width:0">
                <p class="page-title">${escapeHtml(activePanelMeta.title)}</p>
                <p class="page-subtitle">${escapeHtml(activePanelMeta.subtitle)} · menu khusus dari sidebar kiri</p>
              </div>
              <div class="header-actions">${memberIcon}${systemIcon}</div>
            </div>
            <div id="settings-summary" class="page-in s1" style="padding-bottom:18px"></div>
          </div>`;
          renderSettingsSummary();
          return;
        }
  ...
  window.toggleAppViewMode = toggleAppViewMode;
  window.applyAppViewMode = applyAppViewMode;
  window.getAppViewMode = getAppViewMode;
  window.login = login;
  window.toggleAdminMoreMenu = toggleAdminMoreMenu;
  window.openAdminSidebarPanel = openAdminSidebarPanel;
  window.openNormalSettings = openNormalSettings;
  window.openAdminSidebarLogs = openAdminSidebarLogs;
  ...
  window.toggleHeaderGuide = toggleHeaderGuide;
  window.saveHeaderGuideNote = saveHeaderGuideNote;
  window.resetHeaderGuideNoteInput = resetHeaderGuideNoteInput;
  ...
  window.openStaffDailyNoteLink = openStaffDailyNoteLink;
  ...
  async function refreshAllDataNow({ silent = false } = {}) {
    ...
    const txQueryFresh = isAdmin()
      ? query(collection(db, 'transactions'), orderBy('createdAtMs', 'desc'), limit(TX_SYNC_LIMIT_ADMIN))
      : query(collection(db, 'transactions'), where('user', '==', sanitizeUsername(currentUser.username)), orderBy('createdAtMs', 'desc'), limit(TX_SYNC_LIMIT_STAFF));
    const txQueryFallback = isAdmin()
      ? query(collection(db, 'transactions'), limit(TX_SYNC_LIMIT_ADMIN))
      : query(collection(db, 'transactions'), where('user', '==', sanitizeUsername(currentUser.username)), limit(TX_SYNC_LIMIT_STAFF));
    const attendanceQuery = isAdmin()
      ? query(collection(db, 'attendance'), limit(ATT_SYNC_LIMIT_ADMIN))
      : query(collection(db, 'attendance'), where('user', '==', sanitizeUsername(currentUser.username)), limit(ATT_SYNC_LIMIT_STAFF));
    const manualBonusQuery = isAdmin()
      ? query(collection(db, 'manualBonuses'), limit(MANUAL_BONUS_SYNC_LIMIT))
      : query(collection(db, 'manualBonuses'), where('user', '==', sanitizeUsername(currentUser.username)), limit(MANUAL_BONUS_SYNC_LIMIT));

    const [txSnap, attSnap, closingSnap, bonusSnap, guideNoteSnap, staffDailyNoteSnap, receiptTextSnap, manualBonusSnap] = await Promise.all([
      ...
      getDocFreshSafe('Catatan home staff', doc(db, 'closings', STAFF_DAILY_NOTE_DOC_ID)).then(countError),
    ...
    if (staffDailyNoteSnap) {
      cachedStaffDailyHomeNote = staffDailyNoteSnap.exists()
        ? { id: staffDailyNoteSnap.id, note: DEFAULT_STAFF_DAILY_NOTE, enabled: true, ...(staffDailyNoteSnap.data() || {}) }
        : { id: STAFF_DAILY_NOTE_DOC_ID, note: DEFAULT_STAFF_DAILY_NOTE, enabled: true, updatedAtMs: 0, updatedBy: '', updatedByName: '' };
    }
  ...
  // Gear sistem sudah dihapus; guard supaya tombol ESC tidak memanggil fungsi lama yang sudah tidak ada.
...
/* === Inline script 2 dari file asli === */
// Helper tab navigasi halaman laporan (hanya UI, tidak menyentuh Firebase/logika data)
  window.__reportTab = function(btn, sectionId) {
    var tabs = btn.closest('.report-section-tabs');
    if (tabs) tabs.querySelectorAll('.report-stab').forEach(function(b){ b.classList.remove('active'); });
  ...
  // Helper search harian di laporan
  window.__repDailySearch = function(q) {
    var term = (q || '').toLowerCase().trim();
    var rows = document.querySelectorAll('#rep-daily-list .report-daily-row');
...
/* === Inline script 3 dari file asli === */
// UI helper: hanya menambah class layout untuk sidebar desktop / mode Android, tidak menyentuh Firebase/logika data.
  (function(){
    var KEY = 'rocky_admin_view_mode_v1';
    function readMode(){
      try {
        var saved = localStorage.getItem(KEY);
        if(saved === 'desktop' || saved === 'android') return saved;
      } catch(e) {}
      return window.matchMedia('(max-width: 759px)').matches ? 'android' : 'desktop';
    }
    function syncKasirPintarLayout(){
      var app = document.getElementById('app');
      var nav = document.getElementById('bottom-nav');
      if(!app || !nav) return;
      var mode = readMode();
      var visible = window.getComputedStyle(nav).display !== 'none';
      var wide = window.matchMedia('(min-width: 760px)').matches;
      document.documentElement.setAttribute('data-view-mode', mode);
      app.classList.toggle('mode-android', mode === 'android');
      app.classList.toggle('mode-desktop', mode === 'desktop');
      app.classList.toggle('with-sidebar', visible && wide && mode === 'desktop');
    }
    window.syncKasirPintarLayout = syncKasirPintarLayout;
    function boot(){
      var nav = document.getElementById('bottom-nav');
      if(nav && window.MutationObserver){
        new MutationObserver(syncKasirPintarLayout).observe(nav,{attributes:true,attributeFilter:['style','class']});
      }
      window.addEventListener('resize', syncKasirPintarLayout,{passive:true});
      window.addEventListener('storage', syncKasirPintarLayout,{passive:true});
      setTimeout(syncKasirPintarLayout,0);
      setTimeout(syncKasirPintarLayout,300);
    }
    if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', boot); else boot();
  })();

/* === Inline script 4 dari file asli === */
// Mobile Android mode: gunakan sidebar drawer, bukan nav bawah.
  (function(){
    function isSidebarDrawerMode(){
      var mode = document.documentElement.getAttribute('data-view-mode');
      var small = window.matchMedia && window.matchMedia('(max-width: 759px)').matches;
      return mode === 'android' || (mode === 'desktop' && small);
    }
    function appEl(){ return document.getElementById('app'); }
    function navEl(){ return document.getElementById('bottom-nav'); }
    ...
    function syncMobileSidebarState(){
      if(!isSidebarDrawerMode()) closeMobileSidebar(true);
    }
    ...
    window.toggleMobileSidebar = function(){
      var app = appEl();
      if(!app) return;
      if(app.classList.contains('mobile-sidebar-open')) closeMobileSidebar(true);
    ...
    function boot(){
      ...
      if(nav){
        nav.addEventListener('click', function(e){
          ...
          if(btn) setTimeout(function(){ closeMobileSidebar(true); }, 10);
        });
      }
      document.addEventListener('keydown', function(e){
        if(e.key === 'Escape') closeMobileSidebar(true);
      });
      window.addEventListener('resize', syncMobileSidebarState, { passive:true });
      window.addEventListener('orientationchange', syncMobileSidebarState, { passive:true });
      window.addEventListener('storage', syncMobileSidebarState, { passive:true });
      setTimeout(syncMobileSidebarState, 0);
      setTimeout(syncMobileSidebarState, 300);
    }
    if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', boot); else boot();
  })();
